...
class MintService {
    ...
    async mintBundleOnData(event){
        console.log("event Mint mn: ", event);
        console.log("");
        console.log("");
        console.log("");
        ...
        try{
            upgradeInfo = await buildingService.getUpgradeBytypeAndLevel(5, 1);
        }catch(error){
        ...
        try{
            responseTrait = await buildingService.setRandomTrait(response.insertId , 5);
        }catch(error){
        ...
        try{
            response = await buildingService.mintBasicRod(
                player
            );
        }catch(error){
    ...
    async mintBundleOnChange(change){
        console.log("Change in mintBundleOnChange mn: ", change);
    }

    async mintBundleOnError(error){
        console.log("Error in mintBundleOnError mn: ", error);
    }

    async mintOnData(event){
        console.log("event Transfer mn: ", event);
        console.log("");
        console.log("");
        console.log("");
        ...
        if(from == serverConfig.chain.contracts.STAKER_MINER_ADDRESS.trim() || to == serverConfig.chain.contracts.STAKER_MINER_ADDRESS.trim()){
        ...
        if(from == serverConfig.chain.contracts.STAKER_MINER_ADDRESS_OLD.trim() || to == serverConfig.chain.contracts.STAKER_MINER_ADDRESS_OLD.trim()){
            console.log("from or to staking contract");
            console.log("from: ", from);
            console.log("to: ", to);
            return;
        ...
        try{
            bld = await buildingService.getBuilding(tokenId, 5);
        }catch(err){
        ...
        try{
            ...
            console.log("response changeOwnership mn: ", response);

            response = await buildingService.changeOwnershipBasicRod(to, tokenId, TYPE);
            console.log("response changeOwnershipBasicRod mn: ", response);
    ...
    async mintOnChange(change){
        console.log("Change in mintOnChange mn: ", change);
    }

    async mintOnError(error){
        console.log("Error in mintOnError mn: ", error);
    }
}

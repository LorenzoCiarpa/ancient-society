...
class StakerTownhall{
    constructor(){}

    async stakeOnData(event){
        console.log("data stake th: ", event);
        let res;
        let nfts;
        let newPosition = 1;
        let found = false;
        ...
        try{
            res = await stakerService.setStakeQuery(resValues, 1, 1, newPosition);
        ...
        try{
            res = await stakerService.setIsOldContract(resValues.tokenId, 1, true);
    ...
    async unstakeOnData(event){
        console.log("data unstake th: ", event);
        let res;
        let resClaim;
        let resValues = event.returnValues;
        let newStoredResponse;

        newStoredResponse = await dropService.calculateNewStoredResources(resValues.tokenId, 1);
        ...
        try{
            res = await stakerService.setStakeQuery(resValues, 1, 0, 0);
            console.log("response in mint..: ", res);
            // log.info("response in ..:", res);
        }catch(err){
            console.log("error in mint..: ", err);
        ...
        try{
            res = await stakerService.setIsOldContract(resValues.tokenId, 1, false);
            console.log("response in setIsOldContract..: ", res);
            // log.info("response in ..:", res);
        }catch(err){
            console.log("error in setIsOldContract..: ", err);
    ...
    async stakeOnDataNew(event){
        console.log("data stake th new: ", event);
        let res;
        let nfts;
        let newPosition = 1;
        let found = false;
        ...
        let owner;
        try{
            owner = await stakerService.getOwnerByIdAndType(resValues.tokenId, 1);
        }catch(err){
        ...
        let stakedMiners
        try{
            stakedMiners = await stakerService.getStakedNFTByType(owner, 1);
        }catch(err){
    ...
    async unstakeOnDataNew(event){
        ...
        let resClaim;
        try{
            resClaim = await stakerService.makeClaimUnstake(resValues, 1);
        }catch(error){
            console.log("resClaim: error", error)
        }
        ...
        try{
            res = await stakerService.unequipTool(resValues.tokenId, 1);
    ...
    async stakeOnChange(event){
        console.log("change stake th: ", event);
    }
    
    async unstakeOnChange(event){
        console.log("change unstake th: ", event);
        
    }
    
    async stakeOnError(event){
        console.log("error stake th: ", event);
    
    }
    
    async unstakeOnError(event){
        console.log("cerrorange unstake th: ", event);
        
    }
}

module.exports = {StakerTownhall}
...
BEGIN
	...
	/*Calcolo endingTime*/		
	DECLARE endingTimeUpgrade INT DEFAULT (SELECT endingTime 
							FROM buildings as b
							WHERE b.id = bld_upgradedId);
	...
	DECLARE maxedBld INT DEFAULT 0;
	
	DECLARE thLevel INT DEFAULT (SELECT level FROM buildings WHERE address = player AND type = 1);
	DECLARE ljLevel INT DEFAULT (SELECT level FROM buildings WHERE address = player AND type = 2);
	DECLARE smLevel INT DEFAULT (SELECT level FROM buildings WHERE address = player AND type = 3);

	DECLARE maxBldLevel INT DEFAULT 10;
	DECLARE bldToMax INT DEFAULT 3;
   ...
   START TRANSACTION;
	...
	IF bld_upgradedId IS NOT NULL THEN
		...
		(
			...
			FROM buildings
			WHERE address = player
			AND level = maxBldLevel
			AND stake = 1
		);

		IF thLevel = maxBldLevel AND ljLevel = maxBldLevel AND smLevel = maxBldLevel THEN
			INSERT IGNORE INTO leaderboard_challenge
				(address, endingTime)
			VALUES
				(player, endingTimeUpgrade);
		END IF;

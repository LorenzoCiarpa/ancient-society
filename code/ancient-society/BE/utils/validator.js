...
class Validator{
    ...
    static isPositiveInteger(str_) {
        ...
        if (Number.isInteger(num) && num > 0 && num < serverConfig.MAX_NUMBER_VALUE) {
          return true;
        }
    ...
    static validateAddress(address){
        ...
        let regexColony=/^0x[a-fA-F0-9_]{42,43}$/;
        return regex.test(address) || regexColony.test(address);
    }
    ...
    static validateTypeVoucher(type){
        if(this.isPositiveInteger(type)){
            type = parseInt(type);

            if( (type >= 1 && type <= 6) || type == 137){
                return true;
            }
        }
        return false;
    ...
    static ValidatePositiveFloat(str_){
        ...
        if(num <= 0 || num > serverConfig.MAX_NUMBER_VALUE){
    ...
    static ValidateFishermanID(id){
        ...
        if(id < 0 || id >= serverConfig.FISHERMAN_SUPPLY){
    ...
    static ValidateInventoryType(inventoryType){
        let elements = ['tool','recipe','item','resource','card','gear'];
    ...
    static isPositiveFloatWithLessThanTwoDecimals(str_){
        ...
        if(num < 0.01 || num > serverConfig.MAX_NUMBER_VALUE){

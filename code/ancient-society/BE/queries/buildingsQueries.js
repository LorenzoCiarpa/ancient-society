...
class BuildingsQueries {
    ...
    static async resetPassive(idPassive, maxStorableActions) {
        ...
        return new Promise((resolve, reject) => {
            ...
            mysql.query(sql, [maxStorableActions, idPassive], (err, rows, fields) => {
                if (err) {
                    logger.error(`Query error: ${Utils.printErrorLog(err)}`);
                    return reject(new Error(err.message));
                }
    ...
    static async resetBuildingTool(idToolInstance) {
        logger.debug('resetBuildingTool start');
        return new Promise((resolve, reject) => {
            let sql = `
            ...
            mysql.query(sql, [idToolInstance], (err, rows, fields) => {
                ...
                if (rows == undefined || rows == null) {
                    ...
                    logger.debug('resetBuildingTool end');
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
    ...
    static async getPassiveMaxAction(buildingType, level) {
        logger.debug('getPassiveMaxAction start');
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT maxAction FROM passive_max_action WHERE buildingType = ? AND level = ?
            `;

            mysql.query(sql, [buildingType, level], (err, rows, fields) => {
                if (err) {
                    logger.error(`Query error: ${Utils.printErrorLog(err)}`);
                    return reject(new Error(err.message));
                }
                if (rows == undefined || rows == null || rows.length == 0) {
                    ...
                    logger.debug('getPassiveMaxAction end');

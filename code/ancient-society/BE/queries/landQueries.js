...
class LandQueries {
    ...
    static async updateTicketSellingStatus(idTicket, address) {
        ...
        return new Promise((resolve, reject) => {
            let sql = `
                ...
                WHERE idTicket = ? AND address = ?
                    `;
            mysql.query(sql, [serverConfig.marketplace.MARKETPLACE_TICKET_ADDRESS, idTicket, address], (err, rows) => {
                if (err) return reject(new Error(err.message));
                if (rows == undefined || rows == null) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
    ...
    static async getSellingTickets(idLandInstance) {
        logger.debug(`getSellingTickets start`);
        return new Promise((resolve, reject) => {
            let sql = `
                    SELECT * FROM land_instance l_i
                    LEFT JOIN land_contract l_c ON l_c.idContract = l_i.idContract
                    LEFT JOIN land_ticket l_t ON l_t.idContract = l_c.idContract
                    LEFT JOIN ticket_marketplace t_m ON t_m.idTicket = l_t.idTicket
                    WHERE l_i.idLandInstance = ? AND l_i.stake = 1
                    AND l_c.contractStatus = 'active' AND l_c.endingTime > CURRENT_TIMESTAMP
                    AND l_t.status = 'onSale'
                    AND t_m.endingTime > CURRENT_TIMESTAMP AND t_m.marketStatus = 'active' AND t_m.toShow = 1
                `;
            mysql.query(sql, [idLandInstance], (err, rows) => {
                if (err) return reject(new Error(err.message));
                if (rows == undefined || rows == null) {
                    ...
                    logger.debug(`getSellingTickets end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async landClaimHistory(obj) {
        ...
        let claimTime = new Date().toISOString().slice(0, -1);

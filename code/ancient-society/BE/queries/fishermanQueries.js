...
class FishermanQueries {
    ...
    static async createPassiveFishing(type, address, idSea, id, quantity, idFisherman, idToolLevel, idToolInstance, idPassiveFishing, actionNumber, qtyBefore, qtyAfter, coolDown, idFish, quantityFish) {
        ...
        let endingTime = (new Date( (nowTime + (serverConfig.cooldownSec.fisherman * 1000) * (coolDown ? 1 : 0))  )).toISOString().slice(0, -1)
    ...
    static async createFishing(type, address, idSea, id, quantity, idFisherman, idToolLevel, idToolInstance, firstIdConsumable, secondIdConsumable, thirdIdConsumable, reduceCoolDown, noCoolDown, qtyBefore, qtyAfter, idFish, quantityFish) {
        let now = new Date();
        let nowTime = now.getTime();
        let startingTime = (new Date(nowTime)).toISOString().slice(0, -1)
        let endingTime = (new Date(nowTime + ((reduceCoolDown ? serverConfig.cooldownSec.fisherman * 0.8 : serverConfig.cooldownSec.fisherman) * (noCoolDown ? 0 : 1) * 1000))).toISOString().slice(0, -1)
        // let endingTime = (new Date(nowTime - 3600).toISOString())
        let sql, params
        if (type == 1) { // item
            sql = `
                INSERT INTO
                    fishing (idSea, idItem, quantityItem, fishingStartingTime, fishingEndingTime, status, address, idToolInstance, idToolLevel, idFisherman, firstIdConsumable, secondIdConsumable, thirdIdConsumable, quantityBefore, quantityAfter, idFish, quantityFish) 
                VALUES
                    (
                        ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                    )
                `
            params = [idSea, id, quantity, startingTime, endingTime, address, idToolInstance, idToolLevel, idFisherman, firstIdConsumable, secondIdConsumable, thirdIdConsumable, qtyBefore, qtyAfter, idFish, quantityFish]
        } else if (type == 2) { // recipe
            sql = `
                INSERT INTO
                    fishing (idSea, idRecipe, quantityRecipe, fishingStartingTime, fishingEndingTime, status, address, idToolInstance, idToolLevel, idFisherman, firstIdConsumable, secondIdConsumable, thirdIdConsumable, quantityBefore, quantityAfter, idFish, quantityFish) 
                VALUES
                    (
                        ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                    )
        ...
        // } 
            ...
            sql = `
                INSERT INTO
                    fishing (idSea, idFish, quantityFish, fishingStartingTime, fishingEndingTime, status, address, idToolInstance, idToolLevel, idFisherman, firstIdConsumable, secondIdConsumable, thirdIdConsumable) 
                VALUES
                    (
                        ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?
                    )
            `
            params = [idSea, id, quantity, startingTime, endingTime, address, idToolInstance, idToolLevel, idFisherman, firstIdConsumable, secondIdConsumable, thirdIdConsumable]
        }

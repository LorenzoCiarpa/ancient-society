...
class InventoryQueries {
    ...
    static async getPvpRecipeRequirements(address, idRecipeInstance) {
        logger.info(`InventoryQueries.getPvpRecipeRequirements START`)
        let sql, params
        sql = `
            SELECT
                rec_ins.quantity AS recipeCount,
                rec.chanceCraft, rec.idGear, rec.idCard ,rec.idItem, rec.itemQuantity,
                c_req.idResourceRequirement,
                c_req.idItemRequirement,
                c_req.idToolRequirement,
                c_req.idRecipeRequirement,
        ...
        return new Promise((resolve, reject) => {
            mysql.query(sql, params, (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.info(`InventoryQueries.getPvpRecipeRequirements
                     END`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
    ...
    static async getChestLoots(idItemInstance) {
        ...
        return new Promise((resolve, reject) => {
            let sql = `
                ...
                UNION    
                SELECT
                    c_l_r.resourceType as id, 'resource' as type, c_l_r.resourceType as name , null as description, null as image, null as rarity,
                    c_l_r.dropProbability, c_l_r.maxQuantity, c_l_r.alpha, c_l_r.beta 
                FROM
                    item_instance AS i_ins 
                    JOIN
                        ...
                        ON c.idItem = i_ins.idItem 
                    JOIN
                        chest_loot_resource AS c_l_r 
                        ON c_l_r.idChest = c.idChest 
                WHERE
                    i_ins.idItemInstance = ?
                `
            mysql.query(sql, [idItemInstance, idItemInstance, idItemInstance ], (err, rows) => {
                if (err) reject(err)
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
    ...
    static async getInventoryInstanceDataFromAddressAndIdInventoryInstanceAndInventoryType(address, idInventoryInstance, inventoryType) {
        ...
        if (inventoryType == 'item') {
            sql = `
                SELECT
                    i_ins.idItemInstance AS id, IF(c.idChest IS NULL, FALSE, TRUE) AS isChest,
                    IF(w.idWithdrawable IS NULL, FALSE, TRUE) AS isWithdrawable,
                    w.maticRatio,
                    'item' AS type,
            ...
            sql = `
                SELECT
                    ...
                    'recipe' AS type,
                    rec_ins.quantity,
                    rec.idRecipe, rec.name, rec.image, rec.description, rec.chanceCraft,rec.toPvp,
                    rec.idGear,rec.idCard,rec.idItem,
                    m.craft, m.view, m.send, m.sell,
    ...
    static async getFarmConsumables(address) {
        logger.debug(`getFarmConsumables start`)
        let sql = `
            SELECT
                i_con.idItemConsumable, i_con.quantity, i_con.effect AS description,
                i.name, i.image 
            FROM
                item_consumable_farming AS i_con_m 
                JOIN
                    item_consumable AS i_con 
                    ON i_con.idItemConsumable = i_con_m.idItemConsumable 
                JOIN
        ...
        return new Promise((resolve, reject) => {
            mysql.query(sql, address, (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.info(`getFarmConsumables end`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })
    ...
    static async setUpgradeBldHistory(Objects) {
        ...
        let idx;
        let upgradeStartTime = new Date().toISOString().slice(0, -1);
    ...
    static async setUpgradeToolHistory(Objects) {
        ...
        let idx;
        let upgradeTime = new Date().toISOString().slice(0, -1);
    ...
    static async getRecipetoPvpGivenIdRecipeInstance(address, idRecipeInstance) {
        logger.info(`getRecipeGivenIdRecipeInstance start`)
        return new Promise((resolve, reject) => {
            let sql = `SELECT *
            FROM recipe_instance
            LEFT JOIN recipe on recipe.idRecipe=recipe_instance.idRecipe
    ...
    static async getToolInstance(address, idToolInstance) {
        logger.info(`getToolInstance start`);
        return new Promise((resolve, reject) => {
            ...
            mysql.query(sql, [address, idToolInstance], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.info(`getToolInstance end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async setStoredActions(idToolInstance, pkBuilding) {
        logger.info(`setStoredActions start`);
        return new Promise((resolve, reject) => {
            let sql = "UPDATE passive SET idToolInstance = ?, storedActions = 0 WHERE pkBuilding = ?";

            mysql.query(sql, [idToolInstance, pkBuilding], (err, rows) => {
                if (err) reject(err);
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
                    ...
                    logger.info(`setStoredActions end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async toolFakeProperty(idToolInstance) {
        ...
        return new Promise((resolve, reject) => {
            ...
            mysql.query(sql, [serverConfig.marketplace.MARKETPLACE_INVENTORY_TOOL_ADDRESS, idToolInstance], (err, rows) => {
                if (err) reject(err);
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
    ...
    static async getRecipeToPvpInstance(address, idRecipe) {
        logger.info(`InventoryQueries getGemRecipesInstance start`)
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    ...
                    'recipe' AS type,
                    rec.name, rec.image, rec.description, rec.chanceCraft,
                    IF(rec.idItem IS NULL, NULL, rec.idItem) AS product1,
                    IF(rec.idCard IS NULL, NULL, rec.idCard) AS product2,
                    IF(rec.idGear IS NULL, NULL, rec.idGear) AS product3,
                    m.craft, m.view, m.send, m.sell,
                    rec.itemQuantity AS productQuantity,
                    c_req.idResourceRequirement, c_req.idItemRequirement, c_req.idToolRequirement, c_req.idRecipeRequirement,
                    IF(res_req.ancien IS NULL, 0, res_req.ancien) AS requiredAncien,

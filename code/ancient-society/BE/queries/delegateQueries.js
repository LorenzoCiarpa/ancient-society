        ...
        return new Promise((resolve, reject) => {
            let sql = `
                UPDATE
                    delegate 
                SET
                    isAllowed = ?, claim = ?, upgrade = ?, marketplace = ?, transfer = ?, profile = ?, hand = ?, inventory = ? 
                WHERE
                    idDelegate = ?
                `

            mysql.query(sql, [delegate.isAllowed, delegate.delegations.claim, delegate.delegations.upgrade, delegate.delegations.marketplace, delegate.delegations.transfer, delegate.delegations.profile, delegate.delegations.hand, delegate.delegations.inventory, delegate.id], (err, rows) => {
                if (err) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject(new Error(err.message))
                }

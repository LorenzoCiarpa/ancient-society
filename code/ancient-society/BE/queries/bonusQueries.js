...
class BonusQueries {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (check[0].idBonusCode != null) {
                for (let i = 0; i < check.length; i++) {
                    sql += 'AND (NOT bonus.idBonusCode = ?)'
                    params.push(check[i].idBonusCode)
                }
            }
    ...
    static async bonusHistory(object) {
        logger.info(`bonusHistory start`);
        return new Promise((resolve, reject) => {
            let sql;
            let params = [
                object.address,
                object.idToolInstance,
                object.idItemConsumableBonus,
                object.type,
                object.effectOn
            ]

...
class ServerQueries {
    ...
    static async getKeyword(keyword){
        logger.debug(`getKeyword start`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT value
                FROM server_constant 
                WHERE keyword = ?
            `;
            mysql.query(sql, [keyword],(err, rows) => {
    ...
    static async getKeywordPVP(keyword){
        logger.debug(`getKeyword start`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT value
                ...
                WHERE keyword = ?
            `;
            mysqlPvp.query(sql, [keyword],(err, rows) => {
                if(err) return reject(new Error(err.message));
                if(rows == undefined || rows == null){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
    ...
    static async getVoucherEnabled(){
        logger.debug(`getVoucherEnabled start`);
        return new Promise((resolve, reject) => {
            let sql = `
            SELECT value
            FROM server_constant 
            WHERE keyword = 'voucher_enabled'`;
            ...
            mysql.query(sql, (err, rows) => {
                ...
                if(rows == undefined || rows == null){
                    ...
                    logger.debug(`getVoucherEnabled end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async deleteActiveReferalByAddress(address){
        logger.debug(`deleteActiveReferalByAddress start`);
        return new Promise((resolve, reject) => {
            let sql = `
            DELETE FROM referal_instance
            WHERE 
                address = ?
            AND 
                rewarded = false
            ...
            mysql.query(sql, address, (err, rows) => {
                ...
                if(rows == undefined || rows == null){
                    ...
                    logger.debug(`deleteActiveReferalByAddress end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async getReferalRewardByIdBrokenMarketplace(idBrokenMarketplace){
        logger.debug(`getReferalRewardByIdBrokenMarketplace start`);
        return new Promise((resolve, reject) => {
            let sql = `
            SELECT *
            FROM
            ...
            mysql.query(sql, [idBrokenMarketplace], (err, rows) => {
                if(err) return reject(new Error(err.message));
                if(rows == undefined || rows == null){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
                    ...
                    logger.debug(`getReferalRewardByIdBrokenMarketplace end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async setReferalInstance(address, addressReferal, idReferalReward){
        logger.debug(`setReferalInstance start`);
        return new Promise((resolve, reject) => {
            let sql = `
            INSERT INTO referal_instance
                (address, addressReferal, idReferalReward, rewarded, referalActiveTime)
            VALUES
                (?, ?, ?, 0, current_timestamp)
            ;`;

            mysql.query(sql, [address, addressReferal, idReferalReward], (err, rows) => {
                if(err) return reject(new Error(err.message));
                if(rows == undefined || rows == null){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
                    ...
                    logger.debug(`setReferalInstance end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });

...
class UserQueries{
    ...
    static async isNewAccount(address){
        logger.info(`isNewAccount start`);
        return new Promise((resolve, reject) => {
            let sql = `
            SELECT 
                IF(createdAt > (SUBDATE(CURRENT_TIMESTAMP, INTERVAL 1 DAY)), TRUE, FALSE) isNewAccount
            FROM utente 
            WHERE address = ?`;
    ...
    static async checkColony(address1,address2){
        logger.info(`checkColony start`);
        return new Promise((resolve, reject) => {
            let sql = "SELECT * FROM colony WHERE colonyCity = ? AND mainCity = ?";
    
            mysql.query(sql, [address1,address2], (err, rows) => {
                if(err) reject(err);
                if(rows == undefined){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
    ...
    static async checkColonySameMainCity(address1,address2){
        logger.info(`checkColonySameMainCity start`);
        return new Promise((resolve, reject) => {
            let sql = `SELECT colonyCity FROM colony c3 WHERE colonyCity = ? AND colonyCity in (SELECT colonyCity  FROM colony c WHERE mainCity = (SELECT mainCity FROM colony c2 WHERE colonyCity = ?))`;
            ...
            mysql.query(sql, [address1,address2], (err, rows) => {
                ...
                if(rows == undefined){
                    ...
                    logger.info(`checkColonySameMainCity end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async subAncienPro(address, ancien){
        logger.info(`subAncienPro start`);
        logger.debug(`address: ${address}, ancien: ${ancien}`)
        ...
        return new Promise((resolve, reject) => {
            ...
            mysql.query(sql, [ancien, ancien, address, address], (err, rows) =>{
                if(err) return reject(new Error(err.message));
                if(rows == undefined || rows == null){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({

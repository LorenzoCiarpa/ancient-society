...
class RecipeQueries{
    ...
    static async getRecipeGivenIdRecipe(idRecipe){
    ...
    static async getRecipeInstanceByAddressAndIdRecipeInstance(address, idRecipeInstance){
        logger.info(`getRecipeInstanceByAddressAndIdRecipeInstance start`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT * 
                FROM recipe_instance
                WHERE address = ?
                AND idRecipeInstance = ?
                `;
                    
            mysqlPvp.query(sql, [address, idRecipeInstance], (err, rows) => {
                if(err) reject(err);
                if(rows == undefined){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
                    ...
                    logger.info(`getRecipeInstanceByAddressAndIdRecipeInstance end`);
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
            });
        });
    ...
    static async subRecipeByIdRecipeAndAddress(address, idRecipe, quantity) {
        ...
        return new Promise((resolve, reject) => {
            ...
            mysqlPvp.query(sql, [quantity, quantity, address, idRecipe] , (err, rows) => {
                if(err) reject(err);
                if(rows == undefined){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
    ...
    static async addRecipeByIdRecipeAndAddress(address, idRecipe, quantity) {
        ...
        return new Promise((resolve, reject) => {
            ...
            mysqlPvp.query(sql, [quantity, idRecipe, address] , (err, rows) => {
                if(err) reject(err);
                if(rows == undefined){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
    ...
    static async addRecipeByIdRecipeInstanceMarket(idRecipeInstance, quantity, idMarketplaceInventory){
        ...
        return new Promise((resolve, reject) => {
            ...
            mysqlPvp.query(sql, [quantity, idRecipeInstance, idMarketplaceInventory, idMarketplaceInventory], (err, rows) => {
                if(err) reject(err);
                if(rows == undefined){
                    logger.error(`query error: ${Utils.printErrorLog(err)}`);
                    return reject({
        ...
        return new Promise((resolve, reject) => {
            let sql = `
                ...
                WHERE
                    idRecipeInstance = ?
                `
            mysqlPvp.query(sql, [quantity, idRecipeInstance], (err, rows) => {
                if (err) return reject(err);
                if (rows == undefined) {
                    return reject({
                        message: "undefined"
    ...
    static async deleteRecipeInstanceByAddressAndIdRecipeInstance(address, idRecipeInstance) {
        logger.debug(`deleteRecipeInstanceByAddressAndIdRecipeInstance start`)
        return new Promise((resolve, reject) => {
            let sql = `
            ...
            mysqlPvp.query(sql, [address, idRecipeInstance], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.debug(`deleteRecipeInstanceByAddressAndIdRecipeInstance end`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })

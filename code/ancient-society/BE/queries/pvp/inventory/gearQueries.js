...
class GearQueries {
    ...
    static async createGearInstanceByAddressIdGear(address, idGear) {
        logger.debug(`createGearInstanceByAddressIdGear start`)
            ...
            let sql = `
                ...
                VALUES
                    (
                        ?,
                        ?,
                        (SELECT idGearLevel FROM gear_level WHERE idGear=? AND level = (SELECT MIN(level) FROM gear_level WHERE idGear = ?)  ),
                        0
                    )
                `
            mysqlPvp.query(sql, [address, idGear, idGear,idGear], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.debug(`createGearInstanceByAddressIdGear end`)
    ...
    static async checkPropertyToUpgrade(idGearInstance, address) {
        logger.info(`checkPropertyToUpgrade start`)
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    g_ins.idGear, g_ins.equipped,
                    g_lev.percentageBuff, g_lev.flatBuff, g_lev.chanceUpgrade, g_lev.level, g_lev.isUpgradable,
                    g_lev.level - 1 AS prevLevel,
                    g_lev.level + 1 AS nextLevel 
                FROM
            ...
            mysqlPvp.query(sql, [idGearInstance, address], (err, rows) => {
                if (err) reject(err)
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
    ...
    static async getGearLevelByIdGearAndLevel(idGear, level) {
        logger.info(`getGearLevelByIdGearAndLevel start`)
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    idGearLevel,
                ...
                WHERE
                    ...
                    AND level = ?
                `
            mysqlPvp.query(sql, [idGear, level], (err, rows) => {
                if (err) reject(err)
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
                    ...
                    logger.info(`getGearLevelByIdGearAndLevel end`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })
    ...
    static async getGearGivenIdGearInstance(idGearInstance) {
        ...
        return new Promise((resolve, reject) => {
            let sql = `
                ...
                FROM
                    gear_instance gi
                LEFT JOIN
                    gear g ON g.idGear = gi.idGear
                LEFT JOIN 
                    gear_level gl ON gl.idGearLevel = gi.idGearLevel
                WHERE
                    idGearInstance = ? 
                `
    ...
    static async checkRequirementsToUpgradeByAddressAndIdGearLevel(address, idGearLevel, consumableIds) {
        logger.info(`checkRequirementsToUpgradeByAddressAndIdGearLevel start`)
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                ...
                WHERE
                    up_req.idGearLevel = ?
                `, params = [address, address, idGearLevel]
            ...
            mysqlPvp.query(sql, params, (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.info(`checkRequirementsToUpgradeByAddressAndIdGearLevel end`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })
    ...
    static async upgradeGearByIdGearInstance(idGearInstance, idGearLevel) {
        logger.info(`upgradeGearByIdGearInstance start`)
        return new Promise((resolve, reject) => {
            let sql = `
                UPDATE
                    gear_instance 
                ...
                mysqlPvp.query(sql, [idGearLevel,idGearInstance], (err, rows) => {
                if (err) reject(err)
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
                    ...
                    logger.info(`upgradeGearByIdGearInstance end`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })
    ...
    static async downgradeGearByIdGearInstance(idGearInstance, idGearLevel) {
        logger.info(`downgradeGearByIdGearInstance start`)
        return new Promise((resolve, reject) => {
            let sql = `
                UPDATE
                    gear_instance 
                ...
                if (rows == undefined) {
                    ...
                    logger.info(`downgradeGearByIdGearInstance end`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })

...
class PassiveQueries {
    ...
    static async getPassiveLevelFromLevel1(buildingType, level) {
        logger.debug(`getPassiveLevelFromLevel1 START`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    * 
                ...
                WHERE
                    ...
                    AND buildingType = ?
                `
            mysql.query(sql, [level, buildingType], (err, rows) => {
                if (err) return reject(err);
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
                    ...
                    logger.debug(`getPassiveLevelFromLevel1 END`)
                    return resolve(JSON.parse(JSON.stringify(rows))[0])
                }
            })
        })
    ...
    static async getEquippedToolDurabilityFromPkBuilding(pkBuilding) {
        logger.debug(`getEquippedToolDurabilityFromPkBuilding START`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    t_ins.durability 
            ...
            mysql.query(sql, [pkBuilding], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.debug(`getEquippedToolDurabilityFromPkBuilding END`)
    ...
    static async getPassiveDataFromPkBuilding(pkBuilding) {
        ...
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    pas.*,
                    p_lev.level AS passiveLevel, p_lev.actionCoolDown, p_lev.isUpgradable 
                FROM
                    passive AS pas 
                    JOIN
    ...
    // get pkBuilding from the address and buildingType
    static async getPkBuildingFromAddress(address, buildingType) {
        logger.debug(`getPkBuildingFromAddress START`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    id 
                ...
                WHERE
                    ...
                    AND stake = 1
                `
            mysql.query(sql, [address, buildingType], (err, rows) => {
                if (err) return reject(err);
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
                    ...
                    logger.debug(`getPkBuildingFromAddress END`)
                    return resolve(JSON.parse(JSON.stringify(rows))[0])
                }
            })
        })
    ...
    // get Miner pkBuilding from the address
    static async getMinerPkBuildingFromAddress(address) {
        logger.debug(`getMinerPkBuildingFromAddress START`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    id 
            ...
            mysql.query(sql, [address], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.debug(`getMinerPkBuildingFromAddress END`)
                    return resolve(JSON.parse(JSON.stringify(rows))[0])
                }
            })
        })
    ...
    // get Farmer pkBuilding from the address
    static async getFarmerPkBuildingFromAddress(address) {
        logger.debug(`getFarmerPkBuildingFromAddress START`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                    id 
            ...
            mysql.query(sql, [address], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.debug(`getFarmerPkBuildingFromAddress END`)
                    return resolve(JSON.parse(JSON.stringify(rows))[0])
                }
            })
        })
    ...
    // requirements to upgrade to level ? for building type ?
    static async getUpgradeRequirements1(address, buildingType, level) {
        logger.debug(`getUpgradeRequirements1 START`);
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT
                ...
                WHERE
                    p_lev.level = ?
                    AND p_lev.buildingType = ?
                `
            mysql.query(sql, [address, address, level, buildingType], (err, rows) => {
                if (err) return reject(err);
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
                    ...
                    logger.debug(`getUpgradeRequirements1 END`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })

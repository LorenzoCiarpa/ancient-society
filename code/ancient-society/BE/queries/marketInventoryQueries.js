...
class MarketInventoryQueries {
    ...
    static async getCheapestInventoriesPvp(address) {
        ...
        UNION
        SELECT DISTINCT
            mar_inv.idGearInstance as inventoryInstanceId, mar_inv.inventoryType, g_ins.idGear AS inventoryId, mar_inv.price AS inventoryPrice,
            g.name AS inventoryName, g.image AS inventoryImage, g.description AS inventoryDesc, g_lev.level AS inventoryLevel 
        FROM
            marketplace_inventory AS mar_inv 
            JOIN
                test_db.gear_instance AS g_ins 
                ON g_ins.idGearInstance = mar_inv.idGearInstance  
            JOIN
                test_db.gear_level AS g_lev 
            ...
            JOIN
                (
                    SELECT
                        g_ins.idGear, g_lev.level,
                        MIN(mar_inv.price) AS minGearPrice 
                    FROM
                        marketplace_inventory AS mar_inv 
                        JOIN
                            test_db.gear_instance AS g_ins 
                        ...
                        JOIN
                            ...
                            ON g_lev.idGearLevel = g_ins.idGearLevel 
                    WHERE
                        mar_inv.status = 1 AND isPvp = 1 AND mar_inv.inventoryType = 'gear' AND mar_inv.endingTime > CURRENT_TIMESTAMP AND owner <> ?
                    GROUP BY
                        g_ins.idGear, g_lev.level
                )
                AS minGearTable 
                ON minGearTable.idGear = g_ins.idGear 
                AND minGearTable.level = g_lev.level 
                AND mar_inv.price = minGearTable.minGearPrice 
        WHERE
            mar_inv.inventoryType = 'gear' AND isPvp = 1 AND mar_inv.endingTime > CURRENT_TIMESTAMP AND mar_inv.status = 1 AND owner <> ?
        UNION
        SELECT DISTINCT
            mar_inv.idCardInstance  as inventoryInstanceId, mar_inv.inventoryType, c_ins.idCard  AS inventoryId, mar_inv.price AS inventoryPrice,
            c.name AS inventoryName, c.image AS inventoryImage, c.description AS inventoryDesc, c_lev.level AS inventoryLevel 
        FROM
            marketplace_inventory AS mar_inv 
            JOIN
                test_db.card_instance AS c_ins 
                ON c_ins.IdCardInstance = mar_inv.idCardInstance  
            JOIN
                test_db.card_level AS c_lev 
            ...
            JOIN
                (
                    SELECT
                        c_ins.idCard, c_lev.level,
                        MIN(mar_inv.price) AS minCardPrice 
                    FROM
                        marketplace_inventory AS mar_inv 
                        JOIN
                            test_db.card_instance AS c_ins 
                        ...
                        JOIN
                            ...
                            ON c_lev.idCardLevel  = c_ins.idCardLevel 
                    WHERE
                        mar_inv.status = 1 AND isPvp = 1 AND mar_inv.inventoryType = 'card' AND mar_inv.endingTime > CURRENT_TIMESTAMP AND owner <> ?
                    GROUP BY
                        c_ins.idCard, c_lev.level
                )
                AS minCardTable 
                ON minCardTable.idCard = c_ins.idCard 
                AND minCardTable.level = c_lev.level 
                AND mar_inv.price = minCardTable.minCardPrice 
        WHERE
            mar_inv.inventoryType = 'card' AND isPvp = 1 AND mar_inv.endingTime > CURRENT_TIMESTAMP AND mar_inv.status = 1 AND owner <> ?
        ORDER BY
            inventoryType ASC,
            inventoryName ASC,
            inventoryId ASC
            `
            mysql.query(sql, [address, address, address, address, address, address, address, address], (err, rows) => {
                if (err) reject(err)
                if (rows == undefined) {
                    logger.error(`query error: ${Utils.printErrorLog(err)}`)
                    return reject({
    ...
    static async getAllListingPvp(address,type) {
        logger.info(`MarketInventoryQueries.getAllListingPvp START`)
        return new Promise((resolve, reject) => {
            let sql = `
                SELECT *
            ...
            mysql.query(sql, [address,type], (err, rows) => {
                ...
                if (rows == undefined) {
                    ...
                    logger.info(`MarketInventoryQueries.getAllListingPvp END`)
                    return resolve(JSON.parse(JSON.stringify(rows)))
                }
            })
        })
    ...
    // static async getTotalListingPvp(address, type, id, level) {
    //     logger.info('getTotalListing queries start');
    ...
    static async createAdItem(address, id, quantity, price, totalPrice, creationTime, endingTime,isPvp) {
        logger.info('createAdItem start');
        return new Promise((resolve, reject) => {
            let sql = `LOCK TABLES marketplace_inventory write;
            INSERT INTO marketplace_inventory (owner, idItem, quantity, inventoryType, price, totalPrice, status, creationTime, endingTime, isPvp) VALUES (?, ?, ?, 'item', ?, ?, 1, ?, ?,?);
            UNLOCK TABLES`

            mysql.query(sql, [address, id, quantity, price, totalPrice, creationTime, endingTime, isPvp], (err, rows) => {
                if (err) return reject(new Error(err.message));
                if (rows == undefined || rows == null) {
                    console.log("null error: ", address);
    ...
    static async createAdRecipe(address, id, quantity, price, totalPrice, creationTime, endingTime, isPvp) {
        logger.info('createAdRecipe start');
        return new Promise((resolve, reject) => {
            let sql = `LOCK TABLES marketplace_inventory write;
            INSERT INTO marketplace_inventory (owner, idRecipe, quantity, inventoryType, price, totalPrice, status, creationTime, endingTime, isPvp) VALUES (?, ?, ?, 'recipe', ?, ?, 1, ?, ?,?);
            UNLOCK TABLES ;`
    ...
    static async getMarketplaceInventorySingleListingPvp(id, buyer) {
        logger.info('getMarketplaceInventorySingleListingPvp start');
        return new Promise((resolve, reject) => {
            let sql = `
            SELECT marketplace_inventory.*,
            item.name as itemName, item.image as itemImage,
            recipe.name as recipeName, recipe.image as recipeImage,
            c.name as cardName, c.image as cardImage, cl.level as cardLevel,
            g.name as gearName, g.image as gearImage, gl.level as gearLevel
            FROM marketplace_inventory
            LEFT JOIN test_db.item ON marketplace_inventory.idItem = item.idItem
            LEFT JOIN test_db.recipe ON marketplace_inventory.idRecipe= recipe.idRecipe
            LEFT JOIN test_db.gear_instance gi ON marketplace_inventory.idGearInstance  = gi.idGearInstance  
            LEFT JOIN test_db.gear g  ON gi.idGear  = g.idGear 
            LEFT JOIN test_db.gear_level gl  ON gl.idGearLevel  = gi.idGearLevel 
            LEFT JOIN test_db.card_instance ci ON marketplace_inventory.idCardInstance = ci.IdCardInstance  
            LEFT JOIN test_db.card c  ON ci.idCard  = c.idCard 
            LEFT JOIN test_db.card_level cl  ON cl.idCardLevel  = ci.idCardLevel
            WHERE idMarketplaceInventory = ?
            AND status = 1
            AND endingTime > CURRENT_TIMESTAMP()
            AND owner <> ?
            ...
            mysql.query(sql, [id, buyer], (err, rows) => {
                ...
                if (rows == undefined || rows == null) {
                    ...
                    logger.info('getMarketplaceInventorySingleListingPvp end');
                    return resolve(JSON.parse(JSON.stringify(rows)));
                }
    ...
    static async createAdGear(address, id, quantity, price, totalPrice, creationTime, endingTime, isPvp) {
        logger.info('createAdGear start');
        return new Promise((resolve, reject) => {
            let sql = `LOCK TABLES marketplace_inventory write;
            INSERT INTO marketplace_inventory (owner, idGearInstance, quantity, inventoryType, price, totalPrice, status, creationTime, endingTime, isPvp) VALUES (?, ?, ?, 'gear', ?, ?, 1, ?, ?,?);
            UNLOCK TABLES`
    ...
    static async createAdCard(address, id, quantity, price, totalPrice, creationTime, endingTime, isPvp) {
        logger.info('createAdCard start');
        return new Promise((resolve, reject) => {
            let sql = `LOCK TABLES marketplace_inventory write;
            INSERT INTO marketplace_inventory (owner, idCardInstance, quantity, inventoryType, price, totalPrice, status, creationTime, endingTime, isPvp) VALUES (?, ?, ?, 'card', ?, ?, 1, ?, ?,?);
            UNLOCK TABLES`

...
function MarketplaceInventory() {
    ...
    const [pvpres, setpvpRes] = useState(null)
    const [inventoryData, setInventoryData] = useState([])
    const [PVPinventoryData, setPVPinventoryData] = useState([])
    ...
    const [buyPVPCartData, setBuyPVPCartData] = useState([])
    const [buyPVPTotalQuantity, setBuyPVPTotalQuantity] = useState(0)
    const [buyPVPTotalPrice, setBuyPVPTotalPrice] = useState(0)

    const [tabNames, setTabNames] = useState(['Normal', 'PVP'])
    const [currentTabIndex, setCurrentTabIndex] = useState(0)

    const headerTabChange = (index) => {
    ...
    useEffect(() => {
        ...
        buyPVPCartData.forEach(data => {
            sumQuantity += data.quantity
            sumPrice += data.totalPrice
        });

        setBuyPVPTotalQuantity(sumQuantity)
        setBuyPVPTotalPrice(sumPrice)
    }, [buyPVPCartData])

    const getResources = async (idDelegateData, idColonyData) => {
    ...
    const getPVPCheapestInventories = async () => {
        axios.post('/api/m1/marketplaceInventory/getCheapestInventoriesPvp', {
            address: walletAccount
        })
            .then(response => {
                if (response.data.success) {

                    let pvpLists = response.data.data
                    setpvpRes({ cards: response.data.data.cards, gears: response.data.data.gears, items: response.data.data.items, recipes: response.data.data.recipes })
                    setPVPinventoryData([...pvpLists.cards, ...pvpLists.gears, ...pvpLists.items, ...pvpLists.recipes])
                    setOnLoading(false)
                }
            })
            .catch(error => {
    ...
    const [idColony, setIdColony] = useState(null)
    useEffect(() => {
        if (isLogged) {
            ...
            console.log(colonyParam);
            idColonyData = colonyParam;
            setIdColony(idColonyData);

            console.log('call basic get apis');
            getResources(idDelegateData, idColonyData)
            getCheapestInventories()
        }
    }, [isLogged])
    ...
    const [currentPVPTab, setCurrentPVPTab] = useState('all')
    const PVPTabChanged = (tabName) => {
        if (currentPVPTab == tabName) {
            tabName = 'all'
        }
        setCurrentPVPTab(tabName)
        if (tabName == 'all') {
            setPVPinventoryData([...pvpres.cards, ...pvpres.gears, ...pvpres.items, ...pvpres.recipes])
        } else {
            setPVPinventoryData([...pvpres[tabName + 's']])
        }
    }

    const [filterValue, setFilterValue] = useState('')
    const [filterPVPValue, setPVPFilterValue] = useState('')
    ...
    const onDoAction = () => {
        ...
        onCloseConfirmModal()
        let buyCartIds = [];
        (currentTabIndex == 0 ? buyCartData : buyPVPCartData).map((data) => { buyCartIds.push(data.id) })
        let url = currentTabIndex == 0 ? '/api/m1/marketplaceInventory/buyResourceAndInventory' : '/api/m1/marketplaceInventory/buyInventoryPvp'
        axios.post(url, {
            address: walletAccount,
            buyIds: buyCartIds,
            market: (currentTabIndex == 0 ? buyCartData[0].market : buyPVPCartData[0].market),
        ...
        <div className="marketplace-inventory-page">
            ...
            <div className='page-content'>
                ...
                <div className='gamePanel'>
                    <Link to={"/game?b=9071234" + (idDelegate == null ? '' : `&delegate=${idDelegate}`) + (idColony == null ? '' : `&colony=${idColony}`)}>
                    ...
                    <div className='body'>
                        <div className='tab-navs'>
                            {tabNames.map((tabName, index) => (
                                <div key={index} className={'tab-nav ' + (currentTabIndex === index ? 'active' : '')} onClick={() => headerTabChange(index)}>{tabName}</div>
                            ))}
                        </div>
                        <div className='page-content'>
                            {currentTabIndex == 0 ?
                                                ...
                                                <div className='detail-info'>
                                                    ...
                                                    <div className='detail-table'>
                                                        ...
                                                        <TableInventoryInfo
                                                            ...
                                                            callback_buyPVPCartData={setBuyPVPCartData}
                                ...
                                <>{!detailVisible && <>
                                    <div className='list-header'>
                                        {!onLoading && <>
                                            ...
                                            <div className='inventory-filter'>
                                                ...
                                                >
                                                    <InputBase
                                                        sx={{ ml: 1, flex: 1, color: 'wheat' }}
                                                        placeholder="Search"
                                                        value={filterPVPValue}
                                                        inputProps={{ 'aria-label': 'Search' }}
                                                        onChange={e => setPVPFilterValue(e.target.value)}
                                                    />
                                                    <IconButton
                                                        type="submit"
                                                        sx={{ p: '10px', color: 'rgba(255,254,235,0.4)' }} aria-label="search"
                                                        onClick={() => {
                                                            console.log(filterPVPValue)
                                                        }}>
                                                        <SearchIcon />
                                                    </IconButton>
                                                </Box>
                                            ...
                                            <div className='inventory-tabs'>
                                                <IconButton className={`inventory-type-tab card ` + (currentPVPTab == 'card' ? ' selected' : '')} onClick={() => PVPTabChanged('card')} aria-label="card">
                                                    <img src={WOOD_IMAGE} />
                                                </IconButton>
                                                <IconButton className={'inventory-type-tab' + (currentPVPTab == 'item' ? ' selected' : '')} onClick={() => PVPTabChanged('item')} aria-label="item">
                                                    <img src={imgItem} />
                                                </IconButton>
                                                <IconButton className={'inventory-type-tab' + (currentPVPTab == 'gear' ? ' selected' : '')} onClick={() => PVPTabChanged('gear')} aria-label="gear">
                                                    <img src={imgTool} />
                                                </IconButton>
                                                <IconButton className={'inventory-type-tab' + (currentPVPTab == 'recipe' ? ' selected' : '')} onClick={() => PVPTabChanged('recipe')} aria-label="recipe">
                                                    <img src={imgRecipe} />
                                                </IconButton>
                                            </div>
                                        </>}
                                ...
                                > */}
                                        <div className='mk-inventory-list'>
                                            {
                                                PVPinventoryData.map((inventory, index) => ((inventory.inventoryName.toLowerCase().includes(filterPVPValue.toLowerCase()) || inventory.inventoryLevel.toString().toLowerCase().includes(filterPVPValue.toLowerCase())) &&
                                                    <div
                                                        className='mk-inventory'
                                                        key={index}
                                                        onClick={(e) => {
                                                    ...
                                                    >
                                                        ...
                                                        <div className='mk-inventory-name'>
                                                            <div className='mk-name'>
                                                                <span className='mk-sname'>{inventory.inventoryName}</span>{(inventory.inventoryType == 'card' || inventory.inventoryType == 'gear') && <span className='mk-inventory-level'> LVL +{inventory.inventoryLevel}</span>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))
                                                ...
                                                <div className='detail-info'>
                                                    <div className='detail-spec'>
                                                        ...
                                                        <BonusView icon={true} info={currentInventory.bonuses} /> */}
                                                            ...
                                                            <div className='inventory-name'><span className='inventory-sname'>{currentInventory.inventoryName}</span>{(currentInventory.inventoryType == 'card' || currentInventory.inventoryType == 'gear') && <span className='inventory-level'> LVL +{currentInventory.inventoryLevel}</span>}</div>
                                                        </div>
        ...
        >
            ...
            <DialogContent>
                <div className='buyInventory'>
                    {(currentTabIndex == 0 ? buyCartData : buyPVPCartData).map((row, index) => (
                    ...
                    <div className='totalQuantity'>Total {(currentTabIndex == 0 ? buyCartData[0]?.name : buyPVPCartData[0]?.name)}: <span className='colorYellowGreen'>{(currentTabIndex == 0 ? buyTotalQuantity : buyPVPTotalQuantity)}</span></div>
                    <div className='totalPrice'>Total Spent: <span className='colorYellowGreen'>{toFixed(currentTabIndex == 0 ? buyTotalPrice : buyPVPTotalPrice)}</span> Ancien</div>
                    {resources.ancien < (currentTabIndex == 0 ? buyTotalPrice : buyPVPTotalPrice) && <div className='alreadyBought colorRed'>
                        Your ancien is not enough.
                    </div>}
                </div>
            </DialogContent>
            <DialogActions>
                {resources.ancien < (currentTabIndex == 0 ? buyTotalPrice : buyPVPTotalPrice) ? <Button onClick={onDoAction} disabled > Buy now </Button> : <Button onClick={onDoAction} autoFocus > Buy now </Button>}
            </DialogActions>
        </ConfirmDialogs.ConfirmationDialog>

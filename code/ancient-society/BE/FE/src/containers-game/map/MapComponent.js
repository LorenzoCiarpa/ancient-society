...
const g_pvpSize = { width: 75, height: 76 }
const g_pvpPos = { x: 800, y: 130 }
const g_defaultMapZoom = 1
const g_mapMaxZoom = 1.8
const g_mapMinZoom = 1.0
const g_mapZoomUnit = 0.05

const g_buildingType = ['', 'townhall', 'lumberjack', 'stonemine', 'fisherman', 'miner', 'farmer', 'PVP']
...
function MapComponent(props) {
    ...
    const [isStakableMN, setIsStakableMN] = useState('stillChecking')
    const [isStakableFR, setIsStakableFR] = useState('stillChecking')
    ...
    const goToPVP = () => {
        serverConfig?.features.bonusSystem?.pvp && (window.location.href = '/pvp')
    }
    ...
    const clickToBuildHandler = async (newBuildingNft, metamask, ERC721, ERCStaking) => {
        ...
        const stakeResult = await BuildingSetStake(newBuildingNft.id, newBuildingNft.type, newBuildingNft.isOldStakerContract, metamask, ERC721, ERCStaking)
        ...
        if (stakeResult[0]) {
            ...
            if (newBuildingNft.type == 5) setIsStakableMN(false)
            if (newBuildingNft.type == 6) setIsStakableFR(false)
    ...
    const checkStaking = (nfts) => {
        ...
        let minerIsStakable = true;
        let farmerIsStakable = true;
        ...
        setIsStakableMN(minerIsStakable);
        setIsStakableFR(farmerIsStakable);
    }
    ...
    const BuildingSetStake = async (id, type, isOldStakerContract, metamask, ERC721, ERCStaking) => {
        ...
        else if (type == 5) { contractAddress = ERCStaking.contractMiner }
        else if (type == 6) { contractAddress = ERCStaking.contractFarmer }
        else return false

        //Initialize the Contract Object (Staking)
        // console.log(
        //     'Props: ',
        //     id, type, metamask, ERC721, ERCStaking
        // )
        // console.log(
        //     'Staking: ',
        //     contractAddress, ERCStaking.ABI, metamask.walletSigner
        // )
        ...
        else if (type == 5) { contractAddress721 = ERC721.contractMiner }
        else if (type == 6) { contractAddress721 = ERC721.contractFarmer }
        else return false
            ...
            await axios({
                ...
                .then(response => {
                    if (response.data.success) {
                        ...
                        if (type == 5) props.gameCallback_newMiner(true)
                        if (type == 6) props.gameCallback_newFarmer(true)
                    }
                })
                .catch(error => {
    ...
    const callback_craftOpen = (type) => {
        playSound('button')
        setNftInfoVisible('null')
        props.gameCallback_handleClickOnMap(type)
    }
    ...
    const [pvpHover, setPvpHover] = useState(false)
    ...
    return (
        <>
            ...
            {serverConfig?.map.announcement
                ? (idDelegate == null && idColony == null && props.cityData.home == 1 && props.announcement) &&
            ...
            {serverConfig?.features.delegation
                ? <>
                    ...
                    {idColony != null &&
                        <div className='city-banner colony'>
                            <span className='bannerDescription'>{windowSize.width > 450 ? 'This is ' : ''}<span className='bannerCityName'>{settings.cityName.split("_")}</span>{windowSize.width > 250 ? ' Colony City' : ''}</span>
                            <IconButton className='returnBtn' aria-label="return" onClick={() => {
                                playSound('button')
                                props.callback_offColony()
                            }}>
                                <KeyboardReturnIcon />
                            </IconButton>
                        </div>}
                    {/* SCHOLARSHIP LINK */}
                    {idDelegate == null && idColony == null &&
                        <div className='scholarshipPanel'>
                            <Link to="/scholarship">
                                <IconButton className='scholarshipBtn' onClick={(e) => e.stopPropagation()}>
                                    <img src={iconScholarship} />
                                </IconButton>
                            </Link>
                        </div>}
                </>
            ...
            <div className={props.isVisible ? 'mapComponent' : 'mapComponent notVisible'}
                ...
                {serverConfig?.features.bonusSystem.pvp
                    ?
                    cityData.home == 1 &&
                    <div className='bonusMark' style={{
                        left: mapPosition.x + g_pvpPos.x * mapSize.width / g_mapSize.width,
                        top: mapPosition.y + g_pvpPos.y * mapSize.height / g_mapSize.height,
                        width: g_pvpSize.width * mapSize.width / g_mapSize.width,
                        height: g_pvpSize.height * mapSize.height / g_mapSize.height
                    }}>
                        <div className='bonusHole'
                            onMouseEnter={() => { setPvpHover(true) }} onMouseLeave={() => { setPvpHover(false) }}
                            onClick={() => [playSound('bonus'), goToPVP()]}>
                        </div>
                        <img draggable={false} className='bonusImg' src={pvpImage} alt={'PVP'} style={
                            pvpHover ?
                                { filter: 'drop-shadow(2px 2px 0 #FFB13B) drop-shadow(-2px -2px 0 #FFB13B) drop-shadow(2px -2px 0 #FFB13B) drop-shadow(-2px 2px 0 #FFB13B)' } :
                                {}}
                        />
            ...
            >
                ...
                <DialogContent dividers={true}>
                    ...
                    {buildingNfts.filter(buildingNft => !buildingNft.stake).map((buildingNft) => (
                        ((stackNftType == 4 && buildingNft.type == 4)
                            || (stackNftType < 4 &&
                                (buildingNft.type < 4
                                    || buildingNft.type > 4))
                        )
                ...
                {buildingNfts.map((buildingPlace, i) => (
                    (buildingPlace.position && buildingPlace.stake) ?
                        <NFT
                            ...
                            callback_UpdateNFTs={props.callback_UpdateNFTs}
                            ...
                            callback_craftOpen={callback_craftOpen}
...
function BuildingPlace({ info, size, mapSize, mapPosition, clickHandler, builders, onPrestige, prestigeAllow }) {
    ...
    return (
        <div className='buildingPlace'
            ...
            {false && prestigeAllow && info.prestige &&

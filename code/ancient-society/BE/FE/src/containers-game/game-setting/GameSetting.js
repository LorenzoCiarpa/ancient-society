...
function GameSetting(props) {
  ...
  const onSave = () => {
    ...
    formData.append('idColony', idColony)
    formData.append('idEmblem', currentEmblem ? currentEmblem.idEmblem : -1)

    axios.post(`/api/m1/profile/setProfile?address=${props.metamask.walletAccount}&idDelegate=${idDelegate}&idColony=${idColony}`, formData)
      .then(response => {

...
function GameFish(props) {
  ...
  const onRodChange = () => {
    // var alert = confirm('Do you want to change your Rod? You will lose all your stored actions.')
    // if (!alert) return
    const rod = JSON.parse(JSON.stringify(selectedRod))
    playSound('touch')
    onCloseConfirmModal()
    setDoingAction(true)
  ...
  const onDoAction = () => {
    ...
    if (passiveInfo.isPassive) {
      ...
      axios.post('/api/m1/fisherman/startFishing', {
        ...
        idColony: idColony,
        idSea: fishingSea.id,
        consumableIds: [fishConsumables[0].id, fishConsumables[1].id, fishConsumables[2].id]
      })
        .then(response => {
          onResetConsumables()
  ...
  const onConsumableClick = (consumType, pconsumable) => {
    ...
    } else if (consumType == 'fish3') {
      setFishConsumables(fishConsumables.map((consumable, index) => (index == 2 ? pconsumable : consumable)))
                    ...
                    >
                      ...
                      <AccordionDetails className='details'>
                        <div className='fish-action-button'>
                          ...
                          {(idDelegate == null || delegationData.hand) && <Button variant="outlined" color="success" onClick={() => !doingAction && onRepairRodClick()}
                            id='repairRodBtn'
                        ...
                        <>
                          ...
                          >
                            ...
                            {consumables.map((consumable, index) => (
                              consumable.id != fishConsumables[0].id && consumable.id != fishConsumables[1].id && consumable.id != fishConsumables[2].id &&
                          ...
                          <div
                            className={'consumableBtn'}
                            onClick={(e) => { onConsumableBtnClick(e, 'fish3') }}
                            id='fishConsumableBtn3'
                            aria-controls={consumableOpen == 'fish3' ? 'fishConsumableMenu3' : undefined}
                            aria-haspopup="true"
                            aria-expanded={consumableOpen == 'fish3' ? 'true' : undefined}
                          >
                            {fishConsumables[2].id != undefined && <img className='consumable-image' src={fishConsumables[2].image}></img>}
                          </div>
                          <Menu
                            id='fishConsumableMenu3'
                            anchorEl={consumableAnchorEl}
                            open={consumableOpen == 'fish3'}
                            onClose={onCloseConsumable}
                            MenuListProps={{
                              'aria-labelledby': 'fishConsumableBtn3',
                            }}
                            anchorOrigin={{
                              vertical: 'bottom',
                              horizontal: 'center',
                          ...
                          >
                            ...
                            {consumables.map((consumable, index) => (
                              consumable.id != fishConsumables[0].id && consumable.id != fishConsumables[1].id && consumable.id != fishConsumables[2].id &&
                              <MenuItem key={index} onClick={() => onConsumableClick('fish3', consumable)}>
                                <img className='consumableImage' src={consumable.image}></img>
                                <div className='consumableDesc'>
                                  <span className='consumableName'>{consumable.name}</span>
                                  <span className='consumableQuantity'>x {consumable.quantity}</span>
      ...
      >
        ...
        <DialogContent>
          ...
          {openConfirmType == 'fish' &&
            <DialogContentText>
              {(passiveInfo.isPassive && passiveInfo.maxPerformableActions == 0) ? <>No performable actions</> : <> Are you sure you want to fish at {fishingSea?.title}?
                ...
                {fishConsumables[2].id != undefined && <><br />{fishConsumables[2].description}</>}
              </>}
            </DialogContentText>
          }
          ...
          {openConfirmType == 'changeConfirm' &&
            <div className='rodChangeConfirmText'>
              Do you want to change your Rod? You will lose all your stored actions.
            </div>
          }
        ...
        {openConfirmType == 'changeConfirm' &&
          <DialogActions>
            <Button onClick={onRodChange} autoFocus> Sure </Button>
          </DialogActions>
        }

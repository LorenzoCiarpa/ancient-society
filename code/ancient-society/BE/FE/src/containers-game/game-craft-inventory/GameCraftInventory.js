...
/* New Game Component UI Assets BEGIN*/
...
/* New Game Component UI Assets END*/
...
function GameCraftInventory(props) {
  const [toolViewInfo, setToolViewInfo] = useState(props.toolViewInfo)
  useEffect(() => { setToolViewInfo(props.toolViewInfo) }, [props.toolViewInfo])
      ...
      .then(response => {
        try {
          if (response.data.success) {
            ...
            if (toolViewInfo != null) {
              onMiniMenuClick('view', { id: toolViewInfo.id, type: 'tool' })
            } else {
              setOnLoading(false)
            }
          } else {
      ...
      .then(response => {
        try {
          if (response.data.success) {
            ...
            setSellHour(1)
            setOpenChestCount(1)
            setTransformWithdrawableCount(1)
  ...
  const onDoAction = () => {
    ...
    switch (actionType) {
      ...
      case 'chest':
        ...
        break;
      case 'withdrawable':
        axios.post('/api/m1/inventory/transformWithdrawable', {
          address: props.metamask.walletAccount,
          idDelegate: idDelegate,
          idColony: idColony,
          idItemInstance: currentInventory.id,
          quantity: transformWithdrawableCount
        })
          .then(response => onDidAction(response))
          .catch(error => console.log(error))
        break;
  ...
  const onDidAction = (response) => {
    ...
    if (response.data.success && confirmActionType == 'withdrawable') {
      props.callback_addVoucher(response.data.data.voucher)
    }
  ...
  const [transformWithdrawableCount, setTransformWithdrawableCount] = useState(1)
              ...
              <>
                        ...
                        <div key={index} className='craft-item-wrapper'>
                          ...
                          >
                            ...
                            <div className={'craft-item-container' + (inventory.isChest ? ' chest' : '')}>
                              ...
                              <div className={'craft-item-name ' + inventory.type}>
                                <span>{inventory.name}{/* {inventory.type == 'tool' ? ' + ' + (inventory.level ? inventory.level : 0) : ''} */}</span>
                              </div>
                              <img className={'craft-item-img ' + inventory.type} src={inventory.image ? inventory.image : '..'}></img>
                              <span className={'craft-item-quantity ' + inventory.type}>{inventory.quantity ? (inventory.type != 'tool' ? inventory.quantity : 'Lvl+' + (inventory.level ? inventory.level : 0)) : '..'}</span>
                            </div>
                          </div>
                          <Menu
                            id={'mini-menu' + index}
                    ...
                    <div className={'detailView'}>
                      <div className='container'>
                        <div className='left-side'>
                          ...
                          {(currentInventory.type != 'item' || currentInventory.isChest == 1 || currentInventory.isWithdrawable == 1 || miniAction !== 'view')
                            && (idDelegate == null || delegationData.inventory)
                            && <span className='inventory-description'>
                              {currentInventory.description}
                            </span>}
                          {/* {(currentInventory.type != 'item' || currentInventory.isWithdrawable == 1)
                            && (idDelegate == null || delegationData.inventory)
                            && <span className='inventory-description'>
                              {currentInventory.description}
                            </span>} */}
                        ...
                        {miniAction !== 'send' && miniAction !== 'sell' &&
                          <div className='right-side'>
                            {(currentInventory.type == 'item' && ((idDelegate != null && !delegationData.inventory) || (currentInventory.isChest == 0 && currentInventory.isWithdrawable == 0))) &&
                            ...
                            {(idDelegate == null || delegationData.inventory) ?
                              <>
                                ...
                                {currentInventory.isWithdrawable == 1 && <>
                                  <div className='action-panel loot-panel'>
                                    <div className='lootDesc'>
                                      You can transform each {currentInventory.name} to {currentInventory.maticRatio} Matic!
                                    </div>
                                  </div>
                                  <div className='action-panel open-panel'>
                                    <div className='actions'>
                                      <div className='multiple-description'>
                                        You can transform {currentInventory.quantity} {currentInventory.name} at this moment
                                      </div>
                                      <input
                                        className='multipleInput'
                                        step={1}
                                        type='number'
                                        value={transformWithdrawableCount}
                                        onKeyPress={(e) => {
                                          if (e.code === 'Minus') {
                                            e.preventDefault();
                                          } else if (e.code === 'NumpadSubtract') {
                                        ...
                                        onChange={(e) => {
                                          if (e.target.value == 0) {
                                            setTransformWithdrawableCount('')
                                          } else {
                                            setTransformWithdrawableCount(Math.max(Math.min(currentInventory.quantity, parseInt(e.target.value)), 1));
                                          }
                                        }}
                                      />
                                      <div className={'actionBtn openBtn' + ((transformWithdrawableCount == '' || transformWithdrawableCount < 1) ? ' notAllowed' : '')}>
                                        <Button variant="contained" onClick={() => onActionBtnClick('withdrawable')}>
                                          Transform
                                        </Button>
                                      </div>
                                    </div>
                            ...
                            <div className='send-panel'>
                              ...
                              <TextField
                                onChange={(e) => {
                                  ...
                                  if (isAddress(address) || isColonyAddress(address) || isENS(address)) {
                ...
                {!detailVisible &&
                  <div className='pagination-panel'>
                    <div className='paginationBtn prevBtn' onClick={goToPrevPage} ></div>
                    <div className='paginationBtn nextBtn' onClick={goToNextPage} ></div>
                  </div>}
              </> : <>
          ...
          <DialogContentText>
            {
              confirmActionType == 'repair' ? <>Are you sure you want to repair?<br />Durability will be {currentInventory.durabilityTotal}.</> :
                confirmActionType == 'chest' ? <>Are you sure you want to open {openChestCount} chests?</> :
                  confirmActionType == 'withdrawable' ? <>Are you sure you want to transform {transformWithdrawableCount}?</> :
                    confirmActionType == 'upgrade' ? <>
                      Are you sure you want to upgrade?<br />
                      {upgradeConsumables[0].id == 1 || upgradeConsumables[1].id == 1 || upgradeConsumables[2].id == 1 ? 'The upgrade can fail but the tool will never downgrade.' : 'The upgrade can fail and the tool could be downgraded.'}
                      <br />
                      Probability of Success: {Math.min((upgradeConsumables[0].id == 2 || upgradeConsumables[1].id == 2 || upgradeConsumables[2].id == 2 ? 10 : 0) + (upgradeConsumables[0].id == 6 || upgradeConsumables[1].id == 6 || upgradeConsumables[2].id == 6 ? 5 : 0) + (upgradeConsumables[0].id == 7 || upgradeConsumables[1].id == 7 || upgradeConsumables[2].id == 7 ? 15 : 0)  + currentInventory.upgrade?.probability, 100)}%
                    </> :
                      ...
                      confirmActionType == 'craft' ? (slotNeed ? 'You should select the tools to be burnt' : <>
                        ...
                        confirmActionType == 'sell' ? <>
                          Do you want to sell {sellQuantity} {currentInventory.name} for {toFixed(sellQuantity * sellUnitCost)} Ancien?
                          {serverConfig?.features.fee.marketplaceFee &&
                            <>
                              <br />
                              You will pay a {serverConfig?.features.fee.marketplaceFeeValue * 100}% (<a style={{ margin: "0px 3px", color: "lime", fontSize: "1.3rem" }}>{format(toFixed(sellQuantity * sellUnitCost * serverConfig?.features.fee.marketplaceFeeValue))}</a>) fee on your sale
                            </>}
                        </> :

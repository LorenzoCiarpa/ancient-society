...
class NFT extends Component {
	...
	run = () => {
		...
		if (remaining === 0 && this.state.nft.upgradeStatus && this.upgradeTimer) {
			...
			// this.state.nft.upgradeStatus == 0;
			this.props.callback_UpdateNFTs();
		}
	}
	//*************************************** 
	//Upgrade -- Timer Funcs (upgradeEndingTime)

	async stakingApproved(id, type, isOldStakerContract, newStatus = 0) {
		// console.log('stakingApproved...', id, type, isOldStakerContract, newStatus)
		...
		if (isOldStakerContract) {
			if (type == 1) { contractAddress = this.ERCStaking.contractTownhallOld }
			else if (type == 2) { contractAddress = this.ERCStaking.contractLumberjackOld }
			else if (type == 3) { contractAddress = this.ERCStaking.contractStonemineOld }
			else if (type == 4) { contractAddress = this.ERCStaking.contractFishermanOld }
			else if (type == 5) { contractAddress = this.ERCStaking.contractMinerOld }
			else if (type == 6) { contractAddress = this.ERCStaking.contractFarmerOld }
			else return false
		} else {
			...
			else if (type == 5) { contractAddress = this.ERCStaking.contractMiner }
			else if (type == 6) { contractAddress = this.ERCStaking.contractFarmer }
			else return false
		}
		...
		else if (type == 5) { contractAddress721 = this.ERC721.contractMiner }
		else if (type == 6) { contractAddress721 = this.ERC721.contractFarmer }
		else return false
					...
					<div className='right'>
						...
						{!this.props.onPopup
							...
							<div className='right-3'>
								{this.state.idDelegate == null && this.props.prestigeAllow && this.state.nft.prestige && <>
									<Button
										style={'btn-prestige'}
										text={'Prestige'}
										onButtonClick={() => {
											this.props.onPrestige(this.state.nft)
										}}
									/>
								</>}
...

								{
									...
									this.state.nft.type > 0 && this.state.nft.type < 4 ?
										...
										: this.state.nft.type == 4 ?
											(this.state.idDelegate == null || this.state.delegationData.hand) && <Button
												style={
													(this.state.nft.stake)
														? 'btn-claim'
														: 'btn-claim disabled'}
												...
												onButtonClick={() => {
													(this.state.nft.stake)
														&& this.props.callback_craftOpen('fish')
												}
												} />

											//Miner
											: this.state.nft.type == 5 ?
												(this.state.idDelegate == null || this.state.delegationData.hand) && <Button
													style={
														(this.state.nft.stake)
															? 'btn-claim'
													...
													onButtonClick={() => {
														(this.state.nft.stake)
															&& this.props.callback_craftOpen('miner')
													}
													} />

												//Farmer
												: this.state.nft.type == 6 ?
													(this.state.idDelegate == null || this.state.delegationData.hand) && <Button
														style={
															(this.state.nft.stake)
																? 'btn-claim'
														...
														onButtonClick={() => {
															(this.state.nft.stake)
																&& this.props.callback_craftOpen('farmer')
														}
														} />
													//ERROR
													: null

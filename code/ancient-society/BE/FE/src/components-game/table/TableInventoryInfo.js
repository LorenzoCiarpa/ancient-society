...
export default function TableInventoryInfo(props) {
    ...
    const currentTabIndex = useMemo(() => props.currentTabIndex, [props.currentTabIndex])
    ...
    const getInventoryPVPListings = async () => {
        setReady(false)

        axios.post('/api/m1/marketplaceInventory/getTotalListingPvp', {
            address: props.walletAccount,
            type: currentInventory.inventoryType,
            id: currentInventory.inventoryId,
            level: currentInventory.inventoryLevel
    ...
    useEffect(() => {
        ...
        if (currentTabIndex == 0) {
            ...
            props.callback_buyPVPCartData(checkedList)
        }
    }, [checkedList]);
    ...
    const ImgCell = ({ row }) => {
        ...
        return <div className='mrkt-res'>
            ...
            {(row.values.inventoryType == 'card' || row.values.inventoryType == 'item') && <>
                <span className='bonus-level'>5</span>
            </>}

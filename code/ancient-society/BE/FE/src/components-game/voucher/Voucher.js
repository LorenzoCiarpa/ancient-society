...
import iconMatic from './icon-matic.png';
...
class Voucher extends Component {
  ...
  constructor(props) {
    ...
    this.contractReward = props.contracts.contractReward,
    this.voucherStatus = props.voucherStatus,
  ...
  async voucherMint() {
    ...
    else if (type == 137) { contractAddress = this.contractReward }
    else return false
  ...
  getVoucherImage() {
    if (this.props.type == 1) return this.props.resourceImage
    if (this.props.type == 2) return this.props.resourceImage
    if (this.props.type == 3) return this.props.resourceImage
    if (this.props.type == 137) return iconMatic
  }

  getVoucherQuantity() {
    if (this.props.type == 1) return this.props.quantity
    if (this.props.type == 2) return this.props.quantity
    if (this.props.type == 3) return this.props.quantity
    if (this.props.type == 137) return ethers.utils.formatEther(this.props.quantity)
  }
  ...
  render() {
    ...
    return (
      <div className='voucher'>
        ...
        <img src={this.getVoucherImage()} />
        <p>{format(this.getVoucherQuantity())}</p>

        <Button
          text={this.props.voucherStatus == 'pending' ? `Pending...` : `Claim`}
          style={` ` + (this.props.voucherStatus == 'pending' ? ' notAllowed' : ' ')}

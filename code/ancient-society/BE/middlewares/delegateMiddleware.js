...
async function isDelegated(req, res, next) {
    ...
    let isColony = req.locals.isColony;
    ...
    if (isColony != null && isColony != undefined && isColony == true) {
        next();
        return;
    }
...
async function isDelegatedHand(req, res, next) {
    let idDelegate = req.body.idDelegate;
    let address = req.locals.address;
    ...
    if (!delegation[0].hand) {
        console.warn("hand ", delegation)

...
class MarketInventoryInterface {
    ...
    static buildFinalResponseInventoryPvp(listing){
        let listingResponse = [];
        ...
        for (let i = 0; i < listing.length; i++) {
            ...
            if (listing[i].inventoryType == 'recipe') {
                ...
                ad.name = listing[i].gearName + " + " + listing[i].gearLevel;
                ad.image = listing[i].gearImage;
                ad.level = listing[i].gearLevel;
                ad.idGearInstance = listing[i].idGearInstance;
            }else if (listing[i].inventoryType == 'card') {

                ad.name = listing[i].cardName + " + " + listing[i].cardLevel;
                ad.image = listing[i].cardImage;
                ad.level = listing[i].cardLevel;
                ad.idCardInstance = listing[i].idCardInstance;
            }

    ...
    static buildBuildingsModel(rows) {
        ...
        for (let i = 0; i < rows.length; i++) {
            if ((rows[i].level) == serverConfig.buildingLevel[rows[i].type]) {
                ...
                console.log("####### PRESIGE COUNTER ######## ", rows[i].prestigeCounter)
                let newNFT = new BuildingsModel.BuildingsModel(
                    ...
                    rows[i].endingTime,
                    true,
                    rows[i].level >= serverConfig.buildingLevel.prestige[rows[i].type],
                    rows[i].position,
                    rows[i].isOldStakerContract,
                    rows[i].prestigeCounter
                );

...
class MarketInventoryValidation{
    ...
    static buyInventoryPvpValidation(req){
        let address = req.locals.address;
        let buyIds = req.body.buyIds;

        if(!Validator.validateInput(address, buyIds)){
            logger.warn(`Warn in validateInput (Input null or undefined), address: ${address}, buyIds: ${JSON.stringify(buyIds)}`);
            return {
                success: false,
                error: {
                    errorMessage: "input null or undefined"
    ...
    static getTotalListingValidation(req){
        ...
        if(!Validator.validateInput(type, id, level,address)){
    ...
    static getTotalListingPvpValidation(req){
        let address = req.locals.address;
        let type = req.body.type;
        let id = req.body.id;
        let level = req.body.level;
        ...
        if(type == "tool"){
            return {
                success: false,
                error: {
                    errorMessage: `type cannot be tool`
                }
            } 
        }
    ...
    static createAdValidation(req) {
        ...
        if(isPvp != null && isPvp != undefined){
            if(!Validator.ValidateStatus(isPvp)){
                logger.warn(`Warn in validateInput unvalid, isPvp: ${isPvp}`);
                return {
                    success: false,
                    error: {
                        errorMessage: `Not a valid isPvp`
                    }
                }
            }
        }

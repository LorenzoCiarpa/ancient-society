...
class DelegateValidation {
    ...
    static updateDelegateValidation(req) {
        ...
        if ( !Validator.validateInput(delegate.delegations.claim, delegate.delegations.upgrade, delegate.delegations.marketplace, delegate.delegations.transfer, delegate.delegations.profile, delegate.delegations.hand, delegate.delegations.inventory) ) {
            logger.warn(`Warn in validateInput (Input null or undefined), delegate: ${JSON.stringify(delegate)}`)
            return {
                success: false,
                error: {

...
async function getWorld(req, res) {
    ...
    for (let land of landListRawData) {
        ...
        let sellingTickets = []
        try {
            sellingTickets = await LandQueries.getSellingTickets(land.idLandInstance);
        } catch (error) {
            logger.error(`Error in LandQueries.getSellingTickets: ${Utils.printErrorLog(error)}`)
            return res
                .status(401)
                .json({
                    success: false,
        ...
        landList.push({
            ...
            hasSellingTickets: sellingTickets.length == 0 ? false : true
        })
    }
...
async function upgradeLand(req, res) {
    ...
    if (land.level >= serverConfig.buildingLevel[land.type]) {
        return res.json({
            success: false,
...
async function getCity(req, res) {
    ...
    logger.debug(`city profile info response :${JSON.stringify(cityInfo)}`)
...
async function revokeTicket(req, res) {
    ...
    if (checkTicket[0].status == "sent") {
        ...
        let durationMilliSec = serverConfig.MIN_REVOKE_SEC * 1000;

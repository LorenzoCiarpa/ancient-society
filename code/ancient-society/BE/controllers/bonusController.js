...
async function elevateBonus(req, res) {
    ...
    let elevateII = false 
    if(idItemConsumableBonus == 11 || idItemConsumableBonus == 12 || idItemConsumableBonus == 13) elevateII = true
    let checkTool;
    try {
    ...
    let elevateObject = {};
    elevateObject.idItemConsumableBonus = idItemConsumableBonus;
    elevateObject.type = 'ELEVATE';
    elevateObject.idToolInstance = idToolInstance;
    elevateObject.address = address;
    elevateObject.effectOn = effectOn;

    try {
        await BonusQueries.bonusHistory(elevateObject);
    } catch (error) {
        logger.error(`Error in BonusQueries bonusHistory: ${Utils.printErrorLog(error)}`)
        return res
            .status(401)
            .json({
                success: false,
    ...
    //with elevateII the elevate is performable with a single bonus
    let maxBonusElevate = 2
    if(elevateII) maxBonusElevate = 1
    ...
    if (idBonusArray.length < maxBonusElevate) {
        return res
            .status(401)
            .json({
                success: false,
    ...
    let bonus = {};
    if(!elevateII){
        if(idBonusArray.length > 1){
            bonus = BonusService.randomizeElevate(idBonusArray);
            if (bonus == -1) {
                return res
                    .status(401)
                    .json({
            ...
            if(!idBonusArray[0].isMin){
                bonus.downgrade = idBonusArray[0]
            }else{
                bonus.downgrade=null
            }
            if(!idBonusArray[0].isMax){
                bonus.upgrade = idBonusArray[0]
            }else{
                bonus.upgrade=null
            }
        ...
        //i set upgrade == null and do and if so that it wont upgrade if i get the right probability on the consumable
        if(upgrade != null){
            try {
        ...
        bonus = BonusService.randomizeElevateII(idBonusArray);
        if (bonus == -1) {
            return res
                .status(401)
                .json({
        ...
        let upgradeInfo
        if(bonus.upgrade != null){
            for(let toUpgrade of bonus.upgrade){
                try {
                    upgradeInfo = await BonusQueries.getBonusGivenIdBonusCode(toUpgrade.idBonusCode, toUpgrade.tier + 1);
                } catch (error) {
                    logger.error(`Error in BonusQueries getBonusGivenIdBonusCode: ${Utils.printErrorLog(error)}`)
                    return res
                        .status(401)
                ...
                logger.debug(`getBonusGivenIdBonusCode response: ${JSON.stringify(upgradeInfo)}`);
                try {
                    await BonusQueries.updateBonusInstance(toUpgrade.idBonusInstance, upgradeInfo[0].idBonus);
                } catch (error) {
                    logger.error(`Error in BonusQueries updateBonusInstance: ${Utils.printErrorLog(error)}`)
                    return res
                        .status(401)
        ...
        let downgradeInfo;
        if (bonus.downgrade != null) {
            for(let toDowngrade of bonus.downgrade){
                try {
                    downgradeInfo = await BonusQueries.getBonusGivenIdBonusCode(toDowngrade.idBonusCode, toDowngrade.tier - 1);
                } catch (error) {
                    logger.error(`Error in BonusQueries getBonusGivenIdBonusCode: ${Utils.printErrorLog(error)}`)
                    return res
                        .status(401)
                ...
                logger.debug(`getBonusGivenIdBonusCode response: ${JSON.stringify(downgradeInfo)}`);
                try {
                    await BonusQueries.updateBonusInstance(toDowngrade.idBonusInstance, downgradeInfo[0].idBonus);
                } catch (error) {
                    logger.error(`Error in BonusQueries updateBonusInstance: ${Utils.printErrorLog(error)}`)
                    return res
                        .status(401)
...
async function enchantTool(req, res) {
    ...
    let enchantObject = {};
    enchantObject.idItemConsumableBonus = idItemConsumableBonus;
    enchantObject.type = 'ENCHANTMENT';
    enchantObject.idToolInstance = idToolInstance;
    enchantObject.address = address;
    enchantObject.effectOn = checkConsumable[0].effectOn;

    try {
        await BonusQueries.bonusHistory(enchantObject);
    } catch (error) {
        logger.error(`Error in BonusQueries bonusHistory: ${Utils.printErrorLog(error)}`)
        return res
            .status(401)
...
async function rerollBonus(req, res) {
    ...
    let rerollObject = {};
    rerollObject.idItemConsumableBonus = idItemConsumableBonus;
    rerollObject.type = 'REROLL';
    rerollObject.idToolInstance = idToolInstance;
    rerollObject.address = address;
    rerollObject.effectOn = checkConsumable[0].effectOn;

    try {
        await BonusQueries.bonusHistory(rerollObject);
    } catch (error) {
        logger.error(`Error in BonusQueries bonusHistory: ${Utils.printErrorLog(error)}`)
        return res
            .status(401)

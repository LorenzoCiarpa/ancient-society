...
async function getCheapestInventoriesPvp(req, res) {
    logger.info(`getCheapestInventoriesPvp START`)
    logger.debug(`ipAddress:${Validator.getIpAddress(req)}`)
    ...
    let response
    try {
        response = await MarketInventoryService.getCheapestInventoriesPvp(address)
    } catch (error) {
        logger.error(`Error in MarketInventoryService.getCheapestInventoriesPvp: ${Utils.printErrorLog(error)}`)
        return res
            .status(401)
            .json({
                success: false,
    ...
    logger.info(`getCheapestInventoriesPvp response: ${JSON.stringify(response)}`)
...
async function buyInventoryPvp(req, res) {
    ...
    let validation;
    validation = MarketInventoryValidation.buyInventoryPvpValidation(req);  //modificare validation
    if (!validation.success) {
        return res
            .status(401)
            .json(validation);
    ...
    logger.info(`buyInventoryPvp address: ${address}, buyIds: ${JSON.stringify(buyIds)}`);

    try {
        response = await MarketInventoryService.buyInventoryPvp(address, buyIds);
    } catch (error) {
        logger.error(`Error in buyResourceAndInventory MarketInventoryService: ${Utils.printErrorLog(error)}`);
        return res
            .status(401)
    ...
    logger.info(`buyInventoryPvp response: ${JSON.stringify({ success: true, data: response })}`)
    logger.info(`buyInventoryPvp END`);
...
async function createAd(req, res) { //aggiornataPvp
    ...
    if(isPvp == null || isPvp == undefined) isPvp = 0 ;

    logger.info(`createAd address: ${address}, inventoryType: ${inventoryType}, id: ${id}, price: ${price}, duration: ${duration}, quantity: ${quantity}, isPvp:${isPvp}`);
    ...
    if(!isPvp){
        ...
        try {
            ...
            logger.debug(`Error in createAd MarketInventoryService: ${Utils.printErrorLog(error)}`);
            return res
                .status(401)
                .json({
                    success: false,
        ...
        if(user.length == 0){
            logger.warn(`The user that is trying to sell does not have an alpha/omega account`);
            return res
                .status(401)
                .json({
                    success: false,
        ...
        try {
            sellValid = await MarketHelper.isSellablePvp(id, inventoryType);
        } catch (error) {
            logger.error(`Error in MarketHelper.isSellable: ${Utils.printErrorLog(error)}`);
            return res
                .status(401)
        ...
        if (!sellValid) {
            ...
            return res
                ...
                .json({
                    success: false,
                    error: {
                        errorMessage: 'Sell is not allowed'
                    }
                })
        }
        try {
            response = await MarketInventoryService.createAdPvp(address, inventoryType, id, price, duration, quantity);
        } catch (error) {
            logger.debug(`Error in createAd MarketInventoryService: ${Utils.printErrorLog(error)}`);
            return res
                .status(401)
...
async function cancelAd(req, res) {
    ...
    if(!listing.isPvp){
        ...
        try {
            response = await MarketInventoryService.cancelAdHandlerPvp(listing, address, id);
        } catch (error) {
            logger.error(`Error in MarketInventoryService cancelAdHandler: ${Utils.printErrorLog(error)}`);
...
async function getTotalListingPvp(req, res) {
    logger.info("getTotalListingPvp START");
    ...
    let validation;
    validation = MarketInventoryValidation.getTotalListingPvpValidation(req);  //modificare validation
    if (!validation.success) {
        return res
            .status(401)
            .json(validation);
    ...
    logger.info(`getTotalListingPvp address: ${address} type: ${type}, id: ${id}`);

    try {
        response = await MarketInventoryService.getTotalListingPvpService(address, type, id, level);
    } catch (error) {
        logger.error(`Error in MarketInventoryService getTotalListingPvpService: ${Utils.printErrorLog(error)}`);
        return res
            .status(401)
            .json({
                success: false,
    ...
    logger.info(`getTotalListingPvp response: ${JSON.stringify({ success: true, data: response })}`)
    logger.info(`getTotalListingPvp END`);
...
module.exports = { buyResourceAndInventory, getTotalListing, getAllListing, getAccountListing, getPersonalHistory, createAd, buyAd, cancelAd, removeAd, getCheapestInventories ,getTotalListingPvp,buyInventoryPvp,getCheapestInventoriesPvp};
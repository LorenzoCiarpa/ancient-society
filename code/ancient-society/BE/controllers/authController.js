...
const changeKeys = schedule.scheduleJob('0 0 0 * * *', () => {
    ...
    serverConfig['SECRET_KEY_OLD_JWT'] = serverConfig['SECRET_KEY_NEW_JWT'];
    serverConfig['SECRET_KEY_NEW_JWT'] = newJwtKey;
    serverConfig['SECRET_KEY_TO_SIGN'] = serverConfig.WELCOME_MESSAGE + " " + newMessageToSign;
});

if (serverConfig.routes.leaderboard.resetAuto) {
    (async function resetLeaderboard() {
        console.log("resetLeaderboard")
        let resetDay;
        try {
            resetDay = await LeaderboardQueries.getResetDay();
        } catch (error) {
            logger.error(`Error in getResetDay: ${Utils.printErrorLog(error)}`);
            return;
        }

        let dropLeaderboard;
        try {
            dropLeaderboard = await LeaderboardQueries.getDropLeaderboard();
        } catch (error) {
            logger.error(`Error in getResetDay: ${Utils.printErrorLog(error)}`);
            return;
        }

        let resetDayFisherman = resetDay[0].value
        let resetDayGeneral = resetDay[1].value

        let dropLeaderboardFisherman = JSON.parse(dropLeaderboard[0].value)
        let dropLeaderboardGeneral = JSON.parse(dropLeaderboard[1].value)

        const resetLeaderboardGeneral = schedule.scheduleJob(`0 0 0 ${resetDayGeneral} * *`, async () => {
            console.log("resetLeaderboard GENERAL")

            try {
                await LeaderboardQueries.dropLeaderboardReward(dropLeaderboardGeneral, 'general');
            } catch (error) {
                logger.error(`Error in dropLeaderboardReward: ${Utils.printErrorLog(error)}`);
                return;
            }

            try {
                await LeaderboardQueries.backupAndResetLeaderboardGeneral();
            } catch (error) {
                logger.error(`Error in backupAndResetLeaderboardGeneral: ${Utils.printErrorLog(error)}`);
                return;
            }

        });

        const resetLeaderboardFisherman = schedule.scheduleJob(`0 0 0 ${resetDayFisherman} * *`, async () => {
            console.log("resetLeaderboard FISHERMAN")

            try {
                await LeaderboardQueries.dropLeaderboardReward(dropLeaderboardFisherman, 'fisherman');
            } catch (error) {
                logger.error(`Error in dropLeaderboardReward: ${Utils.printErrorLog(error)}`);
                return;
            }
            ...
            try {
                await LeaderboardQueries.backupAndResetLeaderboardFisherman();
            } catch (error) {
                logger.error(`Error in backupAndResetLeaderboardFisherman: ${Utils.printErrorLog(error)}`);
                return;
            }
...
const isLogged = (req, res) => {
    ...
    if (jwtTokenResponse == undefined || jwtTokenResponse == null) {
        ...
        return res
            ...
            .json({
                success: false,
                data: {
                    messageToSign: serverConfig.SECRET_KEY_TO_SIGN
                }
            });
    }
    ...
    jwt.verify(jwtToken, serverConfig.SECRET_KEY_NEW_JWT, (err, token) => {
        if (err) {
            //check if was used the old key
            jwt.verify(jwtToken, serverConfig.SECRET_KEY_OLD_JWT, (err, token) => {
...
const getKeyClear = (req, res) => {
    ...
    return res.json({
        success: true,
        data: {
            key: serverConfig.SECRET_KEY_TO_SIGN
        }
    });
}
...
const sendKeyHash = (req, res) => {
    ...
    let recoveredAddress = web3.eth.accounts.recover(serverConfig.SECRET_KEY_TO_SIGN, signature);
    ...
    let jwtToken = jwt.sign({ address: recoveredAddress }, serverConfig.SECRET_KEY_NEW_JWT, {
        expiresIn: "1d"
    });

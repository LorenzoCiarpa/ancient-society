...
async function upgradeNFT(req, res) {//manca il try e catch
    ...
    if (nft.level >= serverConfig.buildingLevel[nft.type]) {
        return res.json({
            success: false,
            error: {
...
async function claim(req, res) {
    ...
    newLastClaim = new Date().toISOString().slice(0, -1);

    if(serverConfig.routes.fee.productionFee && type == 1){
        responseNewStored.newStored -= responseNewStored.newStored * serverConfig.routes.fee.productionFeeValue;  
    }
    ...
    logger.debug(`EXPCLAIM: ${expClaim}`)
    logger.debug(`REMAINEXP: ${remainExp}`)
    ...
    newAmount = Number(resourceAmount) + integerPart;
...
async function doPrestige(req, res) {
    ...
    let newPassiveMaxAction
    try {
        newPassiveMaxAction = await BuildingsQueries.getPassiveMaxAction(buildingType, 1);
    } catch (error) {
        logger.error(`Error in BuildingsQueries.getUpdateModelByTypeAndLevel: ${Utils.printErrorLog(error)}}`);
        return res
            .status(401)
    ...
    if (buildingType == 4 || buildingType == 5 || buildingType == 6) {
        try {
            await BuildingsQueries.resetBuildingTool(buildingInfo.idToolInstance);
        } catch (error) {
            logger.error(`Error in BuildingsQueries.resetBuildingTool: ${Utils.printErrorLog(error)}}`);
            return res
                .status(401)
                .json({
                    success: false,
        ...
        try {
            await BuildingsQueries.resetPassive(buildingInfo.idPassive, newPassiveMaxAction);
        } catch (error) {
...
async function setStake(req, res) {//deprecata
    ...
    try {
        ...
        let lastClaim = new Date().toISOString().slice(0, -1);
...
async function setPassiveOn(req, res) {
    ...
    if (buildingType == 4 || buildingType == 5 || buildingType == 6) {
        try {
            response = await PassiveService.setPassiveOn(address, buildingType)
        } catch (error) {
            logger.error(`Error in PassiveService.setPassiveOn: ${Utils.printErrorLog(error)}}`)
            return res
                .json({
                    success: false,
                    error: {
...
async function setPassiveOff(req, res) {
    ...
    if (buildingType == 4 || buildingType == 5 || buildingType == 6) {
        try {
            response = await PassiveService.setPassiveOff(address, buildingType)
        } catch (error) {
            logger.error(`Error in PassiveService.setPassiveOff: ${Utils.printErrorLog(error)}}`)
            return res
                .json({
                    success: false,
                    error: {
...
async function upgradePassive(req, res) {
    ...
    if (buildingType == 4 || buildingType == 5 || buildingType == 6) {
        try {
            response = await PassiveService.upgradePassive(address, buildingType)
        } catch (error) {
            logger.error(`Error in PassiveService.upgradePassive: ${Utils.printErrorLog(error)}}`)
            return res
                .json({
                    success: false,
                    error: {

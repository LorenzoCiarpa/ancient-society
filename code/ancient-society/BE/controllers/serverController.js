...
async function setReferalInstance(req, res){
    
    let idBrokenMarketplace = req.body.idBrokenMarketplace;
    let addressReferal = req.body.addressReferral;
    let address = req.locals.address;

    let response;

    // try{
    //     response = await ServerQueries.deleteActiveReferalByAddress(address);
        
    // }catch(error){
    //     logger.error(`Error in ServerQueries.deleteActiveReferalByAddress, ${Utils.printErrorLog(error)}`);
    //     return res
    //     .json({
    //         success: false,
    //         error: {
    //             errorMessage: "ServerQueries.deleteActiveReferalByAddress"
    //         }
    //     });
    // }
    // logger.debug(`deleteActiveReferalByAddress response: ${JSON.stringify(response)}`);

    let referalReward
    try{
        referalReward = await ServerQueries.getReferalRewardByIdBrokenMarketplace(idBrokenMarketplace);
        
    }catch(error){
        logger.error(`Error in ServerQueries.getReferalRewardByIdBrokenMarketplace, ${Utils.printErrorLog(error)}`);
        return res
        .json({
            success: false,
            error: {
                errorMessage: "ServerQueries.getReferalRewardByIdBrokenMarketplace"
            }
        });
    }
    logger.debug(`referalReward response: ${JSON.stringify(referalReward)}`);

    try{
        offerList = await ServerQueries.setReferalInstance(address, addressReferal, referalReward[0].idReferalReward);
        
    }catch(error){
        logger.error(`Error in ServerQueries.setReferalInstance, ${Utils.printErrorLog(error)}`);
        return res
        .json({
            success: false,
            error: {

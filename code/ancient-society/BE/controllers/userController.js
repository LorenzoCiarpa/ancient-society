...
async function directTransfer(req, res) {
    ...
    //colony check
    let colony1, colony2, colony3
    let colony = false
    try {
        colony1 = await UserQueries.checkColony(addressReceiver, addressSender);
    } catch (error) {
        logger.error(`Error in UserQueries checkColony: ${Utils.printErrorLog(error)}`);
        return res
            .json({
                success: false,
                error: {
    ...
    try {
        colony2 = await UserQueries.checkColony(addressSender, addressReceiver);
    } catch (error) {
        logger.error(`Error in UserQueries checkColony: ${Utils.printErrorLog(error)}`);
        return res
            .json({
    ...
    try {
        colony3 = await UserQueries.checkColonySameMainCity(addressSender, addressReceiver);
    } catch (error) {
        logger.error(`Error in UserQueries checkColony: ${Utils.printErrorLog(error)}`);
        return res
            .json({
    ...
    if (colony1.length == 1 || colony2.length == 1 || colony3.length == 1) colony = true
    logger.debug(`Colony check results :${JSON.stringify(colony1), JSON.stringify(colony2), JSON.stringify(colony3)}, so colony restults is ${colony}`)
    ...
    if ((!colony) && serverConfig.routes.fee.transferFee && type == 1) {
        quantity -= quantity * serverConfig.routes.fee.transferFeeValue;
    }
    switch (type) {
        case 1:
            try {

...
async function changeRod(req, res) {
    ...
    let response, userTool, idToolLevel, check, idTool, pkBuilding, nftId, actualEquippedRod, seas;
    ...
    try {
        response = await InventoryService.setNewEquippedTool(nftId, TYPE, actualEquippedRod, idToolInstance, pkBuilding);
    } catch (error) {
    ...
    let storedActionsResponse
    try {
        storedActionsResponse = await InventoryQueries.setStoredActions(idToolInstance, pkBuilding);
    } catch (error) {
        logger.error(`Error in InventoryQueries.setStoredActions: ${Utils.printErrorLog(error)}`);
        throw "Error in InventoryQueries.setStoredActions";
    }


    logger.debug(`InventoryQueries.setStoredActions response : ${JSON.stringify(storedActionsResponse)}`);
...
async function startPassiveFishing(req, res) {
    ...
    try {
        ...
        if (cdBonus != undefined) {
            ...
            let randomValue = random.int(0, 99)
            if (randomValue < cdBonus.percentageBoost) {
...
async function startFishing(req, res) {
    ...
    if (consumableIds[0] == 3 || consumableIds[1] == 3 || consumableIds[2] == 3) {
        reduceCoolDown = true

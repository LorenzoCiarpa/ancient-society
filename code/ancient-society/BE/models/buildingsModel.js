...
class BuildingsModel {
    constructor(
        ...
        this.isOldStakerContract = isOldStakerContract;
        this.prestigeCounter = prestigeCounter;
    }
...
class BuildingsService {
    ...
    buildBuildingsModel(rows, upgradeModel) {
        ...
        for (let i = 0; i < rows.length; i++) {
            for (let j = 0; j < upgradeModel.length; j++) {
                ...
                if (rows[i].level == serverConfig.buildingLevel[rows[i].type]) {
    ...
    buildResponseModelNFTProc(responseData) {
        ...
        if (responseData.level == serverConfig.buildingLevel[responseData.type]) {
            let newUpgradeResources = new UpgradeResources();
    ...
    async retrieveImage(responseAccountData) {
        ...
        for (let i = 0; i < responseAccountData.length; i++) {
            ...
            if (responseAccountData[i].level < serverConfig.buildingLevel[responseAccountData[i].type]) {
    ...
    async retrieveImageNFT(responseAccountData) {
        ...
        if (responseAccountData.level < serverConfig.buildingLevel[responseAccountData.type]) {
...
class UpgradeService {
    ...
    async checkUpgradeStatusNFT(responseAccountData, responseUpgradeModel) {
        ...
        if (responseAccountData.stake && responseAccountData.upgradeStatus) {
            ...
            let lastResetLeaderboard = new Date(serverConfig.LAST_RESET_LEADERBOARD);
            ...
            console.log("lastResetLeaderboard: ", lastResetLeaderboard.toISOString().slice(0, -1))
            console.log("serverConfig.LAST_RESET_LEADERBOARD: ", serverConfig.LAST_RESET_LEADERBOARD)
    ...
    async createEndingTime(upgradeTime) {
        ...
        logger.debug(`endingTime: ${endingTime.toISOString().slice(0, -1)}`);
        logger.debug("createEndingTime end");

        return endingTime.toISOString().slice(0, -1);
    }

...
class FishermanService {
    ...
    static async updateFishingTable(address, idSea, fishedItems, fishedRecipes, fishedFishes, idFisherman, rodIdToolLevel, rodIdInstance, consumableIds, reduceCoolDown, noCoolDown, quantityItem, quantityRecipe, quantityFish) {
        ...
        for (var i = 0; i < fishedItems.length; ++i) {
            ...
            try {
                create = await FishermanQueries.createFishing(1, address, idSea, fishedItems[i].idItem, fishedItems[i].quantity, idFisherman, rodIdToolLevel, rodIdInstance, consumableIds[0], consumableIds[1], consumableIds[2], reduceCoolDown, noCoolDown, quantityItem[i].before, quantityItem[i].after, fishDrop, quantityFish)
                if (fishedFishesDrop?.length > 0) fishedFishesDrop.splice(0, 1)
        ...
        for (var i = 0; i < fishedRecipes.length; ++i) {
            ...
            try {
                create = await FishermanQueries.createFishing(2, address, idSea, fishedRecipes[i].idRecipe, fishedRecipes[i].quantity, idFisherman, rodIdToolLevel, rodIdInstance, consumableIds[0], consumableIds[1], consumableIds[2], reduceCoolDown, noCoolDown, quantityRecipe[i].before, quantityRecipe[i].after, fishDrop, quantityFish)
                if (fishedFishesDrop?.length > 0) fishedFishesDrop.splice(0, 1)
        ...
        for (var i = 0; i < fishedFishesDrop.length; ++i) {
            try {
                create = await FishermanQueries.createFishing(4, address, idSea, fishedFishesDrop[i].idSeaFish, quantityFish, idFisherman, rodIdToolLevel, rodIdInstance, consumableIds[0], consumableIds[1], consumableIds[2], reduceCoolDown, noCoolDown)
                logger.debug(`FishermanQueries.createFishing response : ${JSON.stringify(create)}`)
            } catch (error) {
                logger.error(`FishermanQueries.createFishing error : ${Utils.printErrorLog(error)}`)
                throw (error)
    ...
    static async fish(address, idSea, consumableIds, allBonus) {
        ...
        if (consumableIds[0] == 4 || consumableIds[1] == 4 || consumableIds[2] == 4) {
            chancePlus = true
        }

        //DONT UNCOMMENT BUG PLUS IS TOO BIG
        if ( chancePlus ) {
            fishChance[0].chanceRecipe += fishChance[0].chanceRecipe / 10;
        }
        ...
        if (consumableIds[0] == 5 || consumableIds[1] == 5 || consumableIds[2] == 5) {
            doubleDrop = true
        }
            ...
            if (bonus.idItem != null) {
                ...
                if (random.int(0, 99) < bonus.percentageBoost) {
                    ...
                    try {
                        ...
                        logger.debug(`InventoryQueries.getItemAndMenuGivenIdItem response : ${JSON.stringify(drop)}`)
                    } catch (error) {
                        logger.error(`InventoryQueries.getItemAndMenuGivenIdItem error : ${Utils.printErrorLog(error)}`)
                        throw (error)
                    }
                    drop = drop[0]
                    response.items.push(

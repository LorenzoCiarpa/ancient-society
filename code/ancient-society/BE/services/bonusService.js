...
class BonusService {
    ...
    static buildToolResponse(tools) {
        ...
        let prefixElev = 0; let suffixElev = 0 , implicitElev = 0;
        let prefixMin = 0; let suffixMin = 0 , implicitMin = 0;
        ...
        for (let j = 0; j < tools.length; j++) {
            ...
            if (tools[j].idBonus != null) {
                ...
                if (tools[j].bonusType == 'PREFIX') {
                    prefix = prefix + 1;
                    if (tools[j].bonusTier == serverConfig.bonus.MAX_PREFIX_TIER) {
                        prefixElev = prefixElev + 1;
                    }
                    if (tools[j].bonusTier == 1) {
                        prefixMin += 1;
                    }

                } else if (tools[j].bonusType == 'SUFFIX') {
                    suffix = suffix + 1;
                    if (tools[j].bonusTier == serverConfig.bonus.MAX_SUFFIX_TIER) {
                    ...
                    implicit = implicit + 1;
                    if(tools[j].bonusTier == serverConfig.bonus.MAX_IMPLICIT_TIER){
                        implicitElev = implicitElev + 1;
                    }
                    if (tools[j].bonusTier == 1) {
                        implicitMin += 1;
                    }
        ...
        isElevatableII = BonusService.setIsElevatableII(prefix, suffix, implicit, prefixElev, suffixElev, implicitElev, prefixMin, suffixMin, implicitMin);
        toolObject.isElevatableII = isElevatableII;
    ...
    static getBonusInfoForIdToolInstance(toolInfo, effectOn) {
        ...
        if (effectOn == 'SUFFIX') {
            maxTier = serverConfig.bonus.MAX_SUFFIX_TIER;
        } else if (effectOn == 'PREFIX') {
            maxTier = serverConfig.bonus.MAX_PREFIX_TIER;
        } else if (effectOn == 'IMPLICIT'){
            maxTier = serverConfig.bonus.MAX_IMPLICIT_TIER;
        }
    ...
    static randomizeElevateII(idBonusArray) {
        let upgradeThisBonus = [], downgradeThisBonus = [];
        let response = {};

        if (idBonusArray.length == 0) {
            return -1;
        }

        //let check = false
        // for(elem in idBonusArray){
        //     if((!idBonusArray[elem].isMin) && (!idBonusArray[elem].isMax)){
        //         check = true
        //     }
        // }
        // if(check == false) return -1

        for(let elem of idBonusArray){
            if(elem.isMin) upgradeThisBonus.push(elem)
            else if(elem.isMax) downgradeThisBonus.push(elem)
            else{
                if(random.int(0,99)<50){
                    upgradeThisBonus.push(elem)

                }else{
                    downgradeThisBonus.push(elem)
                }
            }
    ...
    static setIsEnchantable(prefix, suffix, implicit) {
        ...
        if (serverConfig.bonus.MAX_PREFIX_BONUS > prefix) {
            isEnchantable.prefix = true;
        } else {
            isEnchantable.prefix = false;
        }

        if (serverConfig.bonus.MAX_SUFFIX_BONUS > suffix) {
            isEnchantable.suffix = true;
        } else {
            isEnchantable.suffix = false;
        }

        if (serverConfig.bonus.MAX_IMPLICIT_BONUS > implicit) {
    ...
    static setIsRollable(prefix, suffix, implicit) {
        ...
        if (prefix <= serverConfig.bonus.MAX_PREFIX_BONUS && prefix != 0) {
            isRollable.prefix = true;
        } else {
            isRollable.prefix = false;
        }

        if (suffix <= serverConfig.bonus.MAX_SUFFIX_BONUS && suffix != 0) {
            isRollable.suffix = true;
        } else {
            isRollable.suffix = false;
        }

        if (implicit <= serverConfig.bonus.MAX_IMPLICIT_BONUS && implicit != 0) {
    ...
    static setIsElevatable(prefix, suffix, prefixElev, suffixElev) {
        ...
        if ((prefix >= 2) && (prefixElev < serverConfig.bonus.MAX_PREFIX_BONUS)) {
            isElevatable.prefix = true;
        } else {
            isElevatable.prefix = false;
        }

        if ((suffix >= 2) && (suffixElev < serverConfig.bonus.MAX_SUFFIX_BONUS)) {
    ...
    static setIsElevatableII(prefix, suffix, implicit, prefixElev, suffixElev, implicitElev, prefixMin, suffixMin, implicitMin) {
        let isElevatableII = {};

        if ((prefix >= 1) && (prefixElev < prefix) && (prefixMin < prefix)) {
            isElevatableII.prefix = true;
        } else {
            isElevatableII.prefix = false;
        }

        if ((suffix >= 1) && (suffixElev < suffix) && (suffixMin < suffix)) {
            isElevatableII.suffix = true;
        } else {
            isElevatableII.suffix = false;
        }

        if ((implicit >= 1) && (implicitElev < implicit) && (implicitMin < implicit)) {
            isElevatableII.implicit = true;
        } else {
            isElevatableII.implicit = false;
        }



        return isElevatableII;
    }

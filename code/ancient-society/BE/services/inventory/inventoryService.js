...
class InventoryService {
    ...
    static async getInventoryInstanceData(address, idInventoryInstance, inventoryType) {
        ...
        if (inventoryType == 'item' && inventoryInstanceRawData[0].isChest == 1) {
            ...
            if(inventoryInstanceRawData[0].toPvp){
                let craftType,craftedId
                if(inventoryInstanceRawData[0].productName != null){
                    logger.error("Pvp recipes can't craft tools");
                    throw("DB error pvp recipe can't craft tools")
                }
                if(inventoryInstanceRawData[0].idCard == null && inventoryInstanceRawData[0].idItem == null){
                    craftType = 'gear'
                    craftedId = inventoryInstanceRawData[0].idGear;
                }if(inventoryInstanceRawData[0].idGear == null && inventoryInstanceRawData[0].idItem == null){
                    craftType = "card"
                    craftedId = inventoryInstanceRawData[0].idCard;
                }if(inventoryInstanceRawData[0].idCard == null && inventoryInstanceRawData[0].idGear == null){
                    craftType = "item"
                    craftedId = inventoryInstanceRawData[0].idItem;
                }
                inventoryInstanceData.isAvailable = {
                    craft: 1
                }
                ...
                switch (craftType) {
                    case "card":
                        try {
                            craftedInfo = await InventoryQueriesPvp.getCardInfoGivenIdCard(craftedId)
                            logger.debug(`InventoryQueriesPvp getCardInfoGivenIdCard response: ${JSON.stringify(craftedInfo)} `)
                        } catch (error) {
                            logger.error(`Error in InventoryQueriesPvp.getCardInfoGivenIdCard: ${Utils.printErrorLog(error)}`)
                            throw error
                        }
                        break;
                    case "item":
                        try {
                            craftedInfo = await InventoryQueriesPvp.getItemInfoGivenIdItem(craftedId)
                            logger.debug(`InventoryQueriesPvp getItemInfoGivenIdItem response: ${JSON.stringify(craftedInfo)} `)
                        } catch (error) {
                            logger.error(`Error in InventoryQueriesPvp.getItemInfoGivenIdItem: ${Utils.printErrorLog(error)}`)
                            throw error
                        }
                        break;
                    case "gear":
                        try {
                            craftedInfo = await InventoryQueriesPvp.getGearInfoGivenIdGear(craftedId)
                            logger.debug(`InventoryQueriesPvp getGearInfoGivenIdGear response: ${JSON.stringify(craftedInfo)}`)
                        } catch (error) {
                            logger.error(`Error in InventoryQueriesPvp.getGearInfoGivenIdGear: ${Utils.printErrorLog(error)}`)
                            throw error
                        }
                        break;
                }
                ...
                inventoryInstanceData.craft = {
                    ...
                    consumables: [],
                    product: {
                        name: craftedInfo.name,
                        image: craftedInfo.image,
    ...
    static async openChest(address, idItemInstance, openCount) {
        ...
        while (openedCount < openCount) {
            ...
            while (lootDrops) {
                ...
                }else if (droppedLoot.type == 'resource') {
                    op = {}
                    let resource
                    if(droppedLoot.name == "ancien"){
                        try {
                            op = await UserQueries.addAncien(address, lootQuantity)
                            logger.debug(`UserQueries.addAncien response : ${JSON.stringify(op)}`)
                        } catch (error) {
                            logger.error(`UserQueries.addAncien error : ${Utils.printErrorLog(error)}`)
                            throw (error)
                        }
                    } else if(droppedLoot.name == "wood"){
                        try {
                            op = await UserQueries.addWood(address, lootQuantity)
                            logger.debug(`UserQueries.addWood response : ${JSON.stringify(op)}`)
                        } catch (error) {
                            logger.error(`UserQueries.addWood error : ${Utils.printErrorLog(error)}`)
                            throw (error)
                        }
                    } else if(droppedLoot.name == "stone"){
                        try {
                            op = await UserQueries.addStone(address, lootQuantity)
                            logger.debug(`UserQueries.addStone response : ${JSON.stringify(op)}`)
                        } catch (error) {
                            logger.error(`UserQueries.addStone error : ${Utils.printErrorLog(error)}`)
                            throw (error)
                        }
                    }
                }
        ...
        for(let d of drops){
            if(d.type == "resource"){
                if(d.name == "ancien") d.image = serverConfig.ANCIEN_IMAGE
                if(d.name == "wood") d.image = serverConfig.WOOD_IMAGE
                if(d.name == "stone") d.image = serverConfig.STONE_IMAGE
            }
        }
    ...
    static async setNewEquippedTool(nftId, type, OldidToolInstance, idToolInstance, pkBuilding) {
        logger.debug(`setNewEquippedTool start`);
        let response;
        console.log("############################")
        console.log("############################")

        console.log("nftId, type, OldidToolInstance, idToolInstance: ", nftId, type, OldidToolInstance, idToolInstance)
        console.log("############################")
        console.log("############################")

        try {
            response = await InventoryQueries.equipAndUnequipTool(OldidToolInstance, idToolInstance, pkBuilding);
        } catch (error) {
    ...
    static async getRecipeLandNPCInstanceData(idRecipe, address) {
        let inventoryInstanceRawData = []
        try {
            inventoryInstanceRawData = await InventoryQueries.getNPCRecipesInstance(address, idRecipe)
            console.log('**************** 1: ', inventoryInstanceRawData)
    ...
    static async getRecipeToPvpInstanceData(idRecipe, address) {
        let inventoryInstanceRawData = []
        try {
            inventoryInstanceRawData = await InventoryQueries.getRecipeToPvpInstance(address, idRecipe)
            console.log('**************** 1: ', inventoryInstanceRawData)
        } catch (error) {
            logger.error(`Error in InventoryQueries getGemRecipesInstance: ${Utils.printErrorLog(error)}`)
            throw error
        ...
        //p1 item p2 card p3 gear
        let craftType,craftedInfo
        if(inventoryInstanceRawData[0].product1 != null){
            craftType = "item"
            try {
                craftedInfo = await InventoryQueriesPvp.getItemInfoGivenIdItem(inventoryInstanceRawData[0].product1)
            } catch (error) {
                logger.error(`Error in InventoryQueries.getRecipeConsumables: ${Utils.printErrorLog(error)}`)
                throw error
            }
        }
        else if(inventoryInstanceRawData[0].product2 != null){
            craftType = "card"
            try {
                craftedInfo = await InventoryQueriesPvp.getCardInfoGivenIdCard(inventoryInstanceRawData[0].product2)
            } catch (error) {
                logger.error(`Error in InventoryQueriesPvp.getCardInfoGivenIdCard: ${Utils.printErrorLog(error)}`)
                throw error
            }
        } else if(inventoryInstanceRawData[0].product3 != null){
            craftType = "gear"
            try {
                craftedInfo = await InventoryQueriesPvp.getGearInfoGivenIdGear(inventoryInstanceRawData[0].product3)
            } catch (error) {
                logger.error(`Error in InventoryQueriesPvp.getCardInfoGivenIdCard: ${Utils.printErrorLog(error)}`)
                throw error
            }
        ...
        inventoryInstanceData.craft = {
            ...
            product: {
                name: craftedInfo.name,
                image: craftedInfo.image,
                quantity: (craftRequirements[0].productQuantity)
            }
        }
